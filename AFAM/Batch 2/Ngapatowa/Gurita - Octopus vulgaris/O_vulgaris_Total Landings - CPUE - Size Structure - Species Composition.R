...
ourfish.Ngapatowa.O_cyanea <- ourfish.Ngapatowa.clean[species%in% "Octopus cyanea",
                                                         .(count = sum(count)),
                                                         by = .(year_month, round_length)] [order(year_month)]
ourfish.Ngapatowa.O_cyanea$year_month <- as.Date(ourfish.Ngapatowa.O_cyanea$year_month)
ourfish.Ngapatowa.O_cyanea.wide <- ourfish.Ngapatowa.O_cyanea %>%
  pivot_wider(names_from = year_month,
              values_from = count,
              values_fill = 0)

ourfish.Ngapatowa.O_cyanea.sorted <- ourfish.Ngapatowa.O_cyanea.wide %>% arrange(round_length)

#===============================================================================
#Data arrangement: agar bisa dibaca sama R
dates <- colnames(ourfish.Ngapatowa.O_cyanea.sorted)[-1]
...
length.series.data <- list(dates = dates,
                           midLengths = ourfish.Ngapatowa.O_cyanea.sorted$round_length,
                           catch = as.matrix(ourfish.Ngapatowa.O_cyanea.sorted[,-1]))
...
max.length = as.numeric(max(ourfish.Ngapatowa.O_cyanea.sorted$round_length))
...
ourfish.Ngapatowa.O_cyanea.long <- ourfish.Ngapatowa.O_cyanea.sorted %>%
  pivot_longer(
    cols = starts_with("20"),
    names_to = "dates",
    values_to = "value")

ourfish.Ngapatowa.O_cyanea.long.2 <- ourfish.Ngapatowa.O_cyanea.long %>%
  uncount(weights = value)

#Raw Data
length.raw <- tibble(Length = ourfish.Ngapatowa.O_cyanea.long.2$round_length)
...
ourfish.O_cyanea.landings.1 <- ourfish.Ngapatowa.clean[species %in% "Octopus cyanea",
                                            .(sum_landings = sum(weight_kg)),
...
ourfish.O_cyanea.landings.1.plot <- ggplot(ourfish.O_cyanea.landings.1, aes(x = date, y = sum_landings)) +
  ...
  geom_text(data = trend.label,
            ...
            #vjust = 5,
            inherit.aes = FALSE) +
  labs(title = "Total Landings O.cyanea in MAR Ngapatowa",
       x = "",
       y = "Total landings(kg)") +
  theme.default.1 +
  theme(
...
landings.Ngapatowa.indo <- ourfish.O_cyanea.landings.1.plot + theme(
  plot.background = element_rect(fill = "transparent", colour = NA), # Makes the entire plot background transparent
  panel.background = element_rect(fill = "transparent", colour = NA) # Makes the panel background transparent
)
ggsave("landings O_cyanea Ngapatowa.svg", plot = landings.Ngapatowa.indo, bg = "transparent", width = 13.33, height = 7.3)


#### CPUE Trends in Southeast Sulawesi ####
# Subset weight data by date, species for total landings
ourfish.O_cyanea.cpue.1 <- ourfish.Ngapatowa.clean[species %in% "Octopus cyanea",
                                                    .(sum_landings = sum(weight_kg)),
...
ourfish.O_cyanea.cpue.1.plot <- ggplot(ourfish.O_cyanea.cpue.1, aes(x = date, y = sum_landings)) +
  ...
  geom_text(data = trend.label,
            ...
            #vjust = 5,
            inherit.aes = FALSE) +
  labs(title = "Catch Per Unit Effort O.cyanea in MAR Ngapatowa",
       x = "",
       y = "CPUE (kg/fisher/trip)") +
  theme.default.1 +
  theme(
...
cpue.Ngapatowa.indo <- ourfish.O_cyanea.cpue.1.plot + theme(
  plot.background = element_rect(fill = "transparent", colour = NA), # Makes the entire plot background transparent
  panel.background = element_rect(fill = "transparent", colour = NA) # Makes the panel background transparent
)
ggsave("cpue O_cyanea Ngapatowa.svg", plot = cpue.Ngapatowa.indo, bg = "transparent", width = 13.33, height = 7.3)
