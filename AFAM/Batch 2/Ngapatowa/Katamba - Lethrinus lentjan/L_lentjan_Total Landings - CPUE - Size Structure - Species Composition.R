...
ourfish.Ngapatowa.L_lentjan <- ourfish.Ngapatowa.clean[species%in% "Lethrinus lentjan",
                                                         .(count = sum(count)),
                                                         by = .(year_month, round_length)] [order(year_month)]
ourfish.Ngapatowa.L_lentjan$year_month <- as.Date(ourfish.Ngapatowa.L_lentjan$year_month)
ourfish.Ngapatowa.L_lentjan.wide <- ourfish.Ngapatowa.L_lentjan %>%
  pivot_wider(names_from = year_month,
              values_from = count,
              values_fill = 0)

ourfish.Ngapatowa.L_lentjan.sorted <- ourfish.Ngapatowa.L_lentjan.wide %>% arrange(round_length)

#===============================================================================
#Data arrangement: agar bisa dibaca sama R
dates <- colnames(ourfish.Ngapatowa.L_lentjan.sorted)[-1]
...
length.series.data <- list(dates = dates,
                           midLengths = ourfish.Ngapatowa.L_lentjan.sorted$round_length,
                           catch = as.matrix(ourfish.Ngapatowa.L_lentjan.sorted[,-1]))
...
max.length = as.numeric(max(ourfish.Ngapatowa.L_lentjan.sorted$round_length))
...
K.fishlife = 0.42 #Input K value from Fish Life script or other literatures/sources
...
ourfish.Ngapatowa.L_lentjan.long <- ourfish.Ngapatowa.L_lentjan.sorted %>%
  pivot_longer(
    cols = starts_with("20"),
    names_to = "dates",
    values_to = "value")

ourfish.Ngapatowa.L_lentjan.long.2 <- ourfish.Ngapatowa.L_lentjan.long %>%
  uncount(weights = value)

#Raw Data
length.raw <- tibble(Length = ourfish.Ngapatowa.L_lentjan.long.2$round_length)
...
ourfish.L_lentjan.landings.1 <- ourfish.Ngapatowa.clean[species %in% "Lethrinus lentjan",
                                            .(sum_landings = sum(weight_kg)),
...
ourfish.L_lentjan.landings.1.plot <- ggplot(ourfish.L_lentjan.landings.1, aes(x = date, y = sum_landings)) +
  ...
  geom_text(data = trend.label,
            ...
            #vjust = 5,
            inherit.aes = FALSE) +
  labs(title = "Total Landings L.lentjan in MAR Ngapatowa",
       x = "",
       y = "Total landings(kg)") +
  theme.default.1 +
  theme(
...
landings.Ngapatowa.indo <- ourfish.L_lentjan.landings.1.plot + theme(
  plot.background = element_rect(fill = "transparent", colour = NA), # Makes the entire plot background transparent
  panel.background = element_rect(fill = "transparent", colour = NA) # Makes the panel background transparent
)
ggsave("landings L_lentjan Ngapatowa.svg", plot = landings.Ngapatowa.indo, bg = "transparent", width = 13.33, height = 7.3)


#### CPUE Trends in Southeast Sulawesi ####
# Subset weight data by date, species for total landings
ourfish.L_lentjan.cpue.1 <- ourfish.Ngapatowa.clean[species %in% "Lethrinus lentjan",
                                                    .(sum_landings = sum(weight_kg)),
...
ourfish.L_lentjan.cpue.1.plot <- ggplot(ourfish.L_lentjan.cpue.1, aes(x = date, y = sum_landings)) +
  ...
  geom_text(data = trend.label,
            ...
            #vjust = 5,
            inherit.aes = FALSE) +
  labs(title = "Catch Per Unit Effort L.lentjan in MAR Ngapatowa",
       x = "",
       y = "CPUE (kg/fisher/trip)") +
  theme.default.1 +
  theme(
...
cpue.Ngapatowa.indo <- ourfish.L_lentjan.cpue.1.plot + theme(
  plot.background = element_rect(fill = "transparent", colour = NA), # Makes the entire plot background transparent
  panel.background = element_rect(fill = "transparent", colour = NA) # Makes the panel background transparent
)
ggsave("cpue L_lentjan Ngapatowa.svg", plot = cpue.Ngapatowa.indo, bg = "transparent", width = 13.33, height = 7.3)
