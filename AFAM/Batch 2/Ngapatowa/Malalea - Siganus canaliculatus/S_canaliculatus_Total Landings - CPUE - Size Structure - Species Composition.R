...
setwd("C:/Users/IrlanAssidiq/OneDrive - Rare/ourfish/AFAM/Ngapatowa")
getwd()
...
ourfish.ses.landings.1.plot <- ggplot(ourfish.ses.landings.1, aes(x = date, y = sum_landings)) +
  ...
  geom_text(data = trend.label,
            aes(x = as.Date("2022-06-01", format("%Y-%m-%d")),
                ...
                label = trend),
            hjust = 0.1,
            color = "#e78828", #Orange #e78828; Hijau #00833e; Tosca #06a7b4 Merah: #B50A37
            size = 6,
            #vjust = 5,
            inherit.aes = FALSE) +
...
library(EnvStats)
quantile(ourfish.Ngapatowa$weight_kg, 0.01)
quantile(ourfish.Ngapatowa$weight_kg, 0.99)

# Number of k value for Rosner Test
num.k.upper.weight <- ourfish.Ngapatowa[weight_kg >= quantile(weight_kg, 0.99), .N]
num.k.lower.weight <- ourfish.Ngapatowa[weight_kg <= quantile(weight_kg, 0.01), .N]
k.value.rosnertest.weight <- num.k.upper.weight + num.k.lower.weight

# Statistical test for outliers using Rosner Test
rosner.test.landings.data <- rosnerTest(ourfish.Ngapatowa$weight_kg, k = k.value.rosnertest.weight)
...
ourfish.Ngapatowa.outlier <- ourfish.Ngapatowa[landings.data.outliers$Obs.Num, ]
...
ourfish.Ngapatowa.clean <- ourfish.Ngapatowa[-landings.data.outliers$Obs.Num, ]

## Box Plot for annual comparison

# Make data for two years period
ourfish.Ngapatowa.clean.boxplot.1 <- ourfish.Ngapatowa.clean[,
                                                     year_period := as.factor("2 Years Period")]

# Make data for Year 1
ourfish.Ngapatowa.clean.boxplot.2 <- ourfish.Ngapatowa.clean[date >= start.date & date <= "2022-10-01"][,
                                                                                                year_period := as.factor("Year 1")]

# Make data for Year 2
ourfish.Ngapatowa.clean.boxplot.3 <- ourfish.Ngapatowa.clean[date >= "2022-08-01" & date <= end.date][,
                                                                                              year_period := as.factor("Year 2")]

# Combine data of two years period with year 1 & 2 data
ourfish.Ngapatowa.clean.boxplot.4 <- rbind(ourfish.Ngapatowa.clean.boxplot.1, ourfish.Ngapatowa.clean.boxplot.2, ourfish.Ngapatowa.clean.boxplot.3)

# Annual CPUE & Catch Value using box plot
ourfish.Ngapatowa.clean.boxplot.5 <- ourfish.Ngapatowa.clean.boxplot.4[,
...
ourfish.Ngapatowa.clean.boxplot.5[,
...
t.test(data = ourfish.Ngapatowa.clean.boxplot.5[!year_period == "2 Years Period"],
...
boxplot.annual.cpue <- 
  ggplot(data = ourfish.Ngapatowa.clean.boxplot.5,
...
ourfish.Ngapatowa.clean.boxplot.6 <- ourfish.Ngapatowa.clean.boxplot.5[,
                                                               .(n_trip = .N,
...
### Average length
ourfish.Ngapatowa.clean$ind_weight_g <- (ourfish.Ngapatowa.clean$weight_kg *1000)/ourfish.Ngapatowa.clean$count
ourfish.Ngapatowa.clean$length <- (ourfish.Ngapatowa.clean$ind_weight_g/ourfish.Ngapatowa.clean$a)^(1/ourfish.Ngapatowa.clean$b)

### average length by family
ourfish.Ngapatowa.prov.ma$ind_weight_g <- (ourfish.Ngapatowa.prov.ma$weight_kg *1000)/ourfish.Ngapatowa.prov.ma$count
ourfish.Ngapatowa.prov.ma$length <- (ourfish.Ngapatowa.prov.ma$ind_weight_g/ourfish.Ngapatowa.prov.ma$a)^(1/ourfish.Ngapatowa.prov.ma$b)


# Subset weight data by date, location, fisher_id, family
ourfish.Ngapatowa.length.fam.1 <- ourfish.Ngapatowa.prov.ma[,
                                                    .(mean_length_fam = mean(length)),
                                                    by = .(date, location, family)][order(date)]

ourfish.Ngapatowa.length.sp.1 <- ourfish.Ngapatowa.prov.ma[,
                                                   .(mean_length_sp = mean(length)),
                                                   by = .(date, location,family, species)][order(date)]

ourfish.Ngapatowa.length.sp.1 <- filter_if(ourfish.Ngapatowa.length.sp.1, is.numeric, all_vars((.) != 0))
table.sp.length <- table(c(ourfish.Ngapatowa.length.sp.1$species,
                           ourfish.Ngapatowa.length.sp.1$mean_length_sp))

# Rename level1_name and ma_name into the same column name to be combined
ourfish.Ngapatowa.combine.1 <- ourfish.Ngapatowa.clean[ , ]
ourfish.Ngapatowa.combine.2 <- ourfish.Ngapatowa.clean[ , ]

# Combine data from whole Ngapatowa with data by MA+R
ourfish.Ngapatowa.prov.ma <- rbind(ourfish.Ngapatowa.combine.1, ourfish.Ngapatowa.combine.2)
...
table.fam <- table(ourfish.Ngapatowa.prov.ma$family)

# Change data into wide format
ourfish.Ngapatowa.length.fam.1.wide <- dcast(ourfish.Ngapatowa.length.fam.1,
                                         formula = date + location ~ family,
                                         value.var = "mean_length_fam")

ourfish.Ngapatowa.length.sp.1.wide <- dcast(ourfish.Ngapatowa.length.sp.1,
                                        formula = date + location + family ~ species,
                                        value.var = "mean_length_sp")

# Replace NA value with 0
ourfish.Ngapatowa.length.fam.1.wide[is.na(ourfish.Ngapatowa.length.fam.1.wide)] <- 0

ourfish.Ngapatowa.length.sp.1.wide[is.na(ourfish.Ngapatowa.length.sp.1.wide)] <- 0

# Change data back into long format
ourfish.Ngapatowa.length.fam.1.long <- melt(ourfish.Ngapatowa.length.fam.1.wide,
                                        measure.vars = c(3:10), # Please check the number of column first
                                        variable.name = "family",
                                        value.name = "mean_length_fam")
ourfish.Ngapatowa.length.fam.1.long.clean <- filter_if(ourfish.Ngapatowa.length.fam.1.long, is.numeric, all_vars((.) !=0))
ourfish.Ngapatowa.length.fam.1.long.clean <- filter_if(ourfish.Ngapatowa.length.fam.1.long.clean, is.numeric, all_vars((.) !=Inf))
ourfish.Ngapatowa.length.sp.1.long <- melt(ourfish.Ngapatowa.length.sp.1.wide,
                                       measure.vars = c(4:19), # Please check the number of column first
                                       variable.name = "species",
                                       value.name = "mean_length_sp")

ourfish.Ngapatowa.length.sp.1.long.clean <- filter_if(ourfish.Ngapatowa.length.sp.1.long, is.numeric, all_vars((.) != 0))
ourfish.Ngapatowa.length.sp.1.long.clean <- filter_if(ourfish.Ngapatowa.length.sp.1.long.clean, is.numeric, all_vars((.) !=Inf)) 

table.sp.clean <- table(ourfish.Ngapatowa.length.sp.1.long.clean$species)

#Size Structure
#creating new column for Year_month
library(zoo)
ourfish.Ngapatowa.clean$year_month <- ourfish.Ngapatowa.clean$date
ourfish.Ngapatowa.clean$year_month <- as.yearmon(ourfish.Ngapatowa.clean$year_month, format = "%Y_%m")

#creating midlength, using round up function
ourfish.Ngapatowa.clean$round_length <- round(ourfish.Ngapatowa.clean$length)

#Katamba - Siganus canaliculatus
ourfish.Ngapatowa.S_canaliculatus <- ourfish.Ngapatowa.clean[species%in% "Siganus canaliculatus",
                                                         .(count = sum(count)),
                                                         by = .(year_month, round_length)] [order(year_month)]
ourfish.Ngapatowa.S_canaliculatus$year_month <- as.Date(ourfish.Ngapatowa.S_canaliculatus$year_month)
ourfish.Ngapatowa.S_canaliculatus.wide <- ourfish.Ngapatowa.S_canaliculatus %>%
  pivot_wider(names_from = year_month,
              values_from = count,
              values_fill = 0)

ourfish.Ngapatowa.S_canaliculatus.sorted <- ourfish.Ngapatowa.S_canaliculatus.wide %>% arrange(round_length)

#===============================================================================
#Data arrangement: agar bisa dibaca sama R
dates <- colnames(ourfish.Ngapatowa.S_canaliculatus.sorted)[-1]
...
length.series.data <- list(dates = dates,
                           midLengths = ourfish.Ngapatowa.S_canaliculatus.sorted$round_length,
                           catch = as.matrix(ourfish.Ngapatowa.S_canaliculatus.sorted[,-1]))
...
max.length = as.numeric(max(ourfish.Ngapatowa.S_canaliculatus.sorted$round_length))
...
K.fishlife = 0.78 #Input K value from Fish Life script or other literatures/sources
...
length.series.data$M = 0.655

#preparing raw data for length series
ourfish.Ngapatowa.S_canaliculatus.long <- ourfish.Ngapatowa.S_canaliculatus.sorted %>%
  pivot_longer(
    cols = starts_with("20"),
    names_to = "dates",
    values_to = "value")

ourfish.Ngapatowa.S_canaliculatus.long.2 <- ourfish.Ngapatowa.S_canaliculatus.long %>%
  uncount(weights = value)

#Raw Data
length.raw <- tibble(Length = ourfish.Ngapatowa.S_canaliculatus.long.2$round_length)
...
ggsave("S_canaliculatus size.svg", plot = size.structure.plot, bg = "transparent", width = 13.33, height = 7.3)

#total landings & CPUE L-lentjan================================================
ourfish.S_canaliculatus.landings.1 <- ourfish.Ngapatowa.clean[species %in% "Siganus canaliculatus",
                                            .(sum_landings = sum(weight_kg)),
                                            by = .(date, species)][order(date)]

ourfish.S_canaliculatus.landings.1.glm <- ourfish.S_canaliculatus.landings.1 %>%
  nest(-'species') %>%
  mutate(model = map(data, ~ glm(sum_landings ~ date, data = .))) %>%
  mutate(tidied = map(model, tidy)) %>%
  unnest(tidied)

ourfish.S_canaliculatus.landings.1.glm2 <- ourfish.S_canaliculatus.landings.1.glm %>%
  dplyr::filter(term == "date") %>%
  mutate(p_value_adjusted = p.adjust(p.value)) %>%
  arrange(p_value_adjusted) %>%
  mutate(stat_test = ifelse(p_value_adjusted < 0.05, "significant", "not significant")) %>% #Pvalue <0.05 indicate ada perbedaan antara data dalam kelompok (signifikan)
...
setDT(ourfish.S_canaliculatus.landings.1.glm2)

# Label Landings trends
ourfish.S_canaliculatus.landings.1.glm3 <- ourfish.S_canaliculatus.landings.1.glm2[,
                                                           trend := case_when(estimate >= 0 & stat_test == "significant" ~ "Increased", #estimate is to measure the alternative hypothesis, >=0 means there is positive linear relationship between predictor (date) and response (CPUE or weight)
                                                                              stat_test == "not significant" ~ "Stable",
                                                                              TRUE ~ "Decreased")]

trend.label <- ourfish.S_canaliculatus.landings.1.glm3[, `:=`(term = NULL, estimate = NULL, std.error = NULL, statistic = NULL, p.value = NULL, p_value_adjusted = NULL, stat_test = NULL)]

summary(ourfish.S_canaliculatus.landings.1.glm3)

ourfish.S_canaliculatus.landings.1.plot <- ggplot(ourfish.S_canaliculatus.landings.1, aes(x = date, y = sum_landings)) +
  geom_smooth(method = "loess", span = 0.5, color = "#e78828", fill = "#e78828") +
  scale_y_continuous(expand = c(0,0), limits = c(0,NA), oob = squish) +
  scale_x_date(labels = date_format("%b %Y"),
               date_breaks = "6 month") +
  geom_text(data = trend.label,
            ...
            #vjust = 5,
            inherit.aes = FALSE) +
  labs(title = "Total Landings S.canaliculatus in MAR Ngapatowa",
       x = "",
       y = "Total landings(kg)") +
  theme.default.1 +
  theme(
...
landings.Ngapatowa.indo <- ourfish.S_canaliculatus.landings.1.plot + theme(
  plot.background = element_rect(fill = "transparent", colour = NA), # Makes the entire plot background transparent
  panel.background = element_rect(fill = "transparent", colour = NA) # Makes the panel background transparent
)
ggsave("landings S_canaliculatus Ngapatowa.svg", plot = landings.Ngapatowa.indo, bg = "transparent", width = 13.33, height = 7.3)


#### CPUE Trends in Southeast Sulawesi ####
# Subset weight data by date, species for total landings
ourfish.S_canaliculatus.cpue.1 <- ourfish.Ngapatowa.clean[species %in% "Siganus canaliculatus",
                                                    .(sum_landings = sum(weight_kg)),
                                                    by = .(date, species, fisher_id)][order(date)]

ourfish.S_canaliculatus.cpue.1.glm <- ourfish.S_canaliculatus.cpue.1 %>%
  nest(-species) %>%
  mutate(model = map(data, ~ glm(sum_landings ~ date, data = .))) %>%
  mutate(tidied = map(model, tidy)) %>%
  unnest(tidied)

ourfish.S_canaliculatus.cpue.1.glm2 <- ourfish.S_canaliculatus.cpue.1.glm %>%
  dplyr::filter(term == "date") %>%
  mutate(p_value_adjusted = p.adjust(p.value)) %>%
  arrange(p_value_adjusted) %>%
  mutate(stat_test = ifelse(p_value_adjusted < 0.05, "significant", "not significant")) %>% #Pvalue <0.05 indicate ada perbedaan antara data dalam kelompok (signifikan)
...
setDT(ourfish.S_canaliculatus.cpue.1.glm2)

# Label Landings trends
ourfish.S_canaliculatus.cpue.1.glm3 <- ourfish.S_canaliculatus.cpue.1.glm2[,
                                                   trend := case_when(estimate >= 0 & stat_test == "significant" ~ "Increased", #estimate is to measure the alternative hypothesis, >=0 means there is positive linear relationship between predictor (date) and response (CPUE or weight)
                                                                      stat_test == "not significant" ~ "Stable",
                                                                      TRUE ~ "Decreased")]

trend.label <- ourfish.S_canaliculatus.cpue.1.glm3[, `:=`(term = NULL, estimate = NULL, std.error = NULL, statistic = NULL, p.value = NULL, p_value_adjusted = NULL, stat_test = NULL)]

summary(ourfish.S_canaliculatus.cpue.1.glm3)

ourfish.S_canaliculatus.cpue.1.plot <- ggplot(ourfish.S_canaliculatus.cpue.1, aes(x = date, y = sum_landings)) +
  geom_smooth(method = "loess", span = 0.5, color = "#e78828", fill = "#e78828") +
  scale_y_continuous(expand = c(0,0), limits = c(0,NA), oob = squish) +
  scale_x_date(labels = date_format("%b %Y"),
               date_breaks = "6 month") +
  geom_text(data = trend.label,
            ...
            #vjust = 5,
            inherit.aes = FALSE) +
  labs(title = "Catch Per Unit Effort S.canaliculatus in MAR Ngapatowa",
       x = "",
       y = "CPUE (kg/fisher/trip)") +
  theme.default.1 +
  theme(
...
cpue.Ngapatowa.indo <- ourfish.S_canaliculatus.cpue.1.plot + theme(
  plot.background = element_rect(fill = "transparent", colour = NA), # Makes the entire plot background transparent
  panel.background = element_rect(fill = "transparent", colour = NA) # Makes the panel background transparent
)
ggsave("cpue S_canaliculatus Ngapatowa.svg", plot = cpue.Ngapatowa.indo, bg = "transparent", width = 13.33, height = 7.3)
