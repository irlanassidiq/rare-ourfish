...
#Katamba - Cephalopholis miniata
ourfish.Masteng.c_miniata <- ourfish.Masteng.clean[species%in% "Cephalopholis miniata",
                                                         .(count = sum(count)),
                                                         by = .(year_month, round_length)] [order(year_month)]
ourfish.Masteng.c_miniata$year_month <- as.Date(ourfish.Masteng.c_miniata$year_month)
ourfish.Masteng.c_miniata.wide <- ourfish.Masteng.c_miniata %>%
  pivot_wider(names_from = year_month,
              values_from = count,
              values_fill = 0)

ourfish.Masteng.c_miniata.sorted <- ourfish.Masteng.c_miniata.wide %>% arrange(round_length)

#===============================================================================
#Data arrangement: agar bisa dibaca sama R
dates <- colnames(ourfish.Masteng.c_miniata.sorted)[-1]
...
length.series.data <- list(dates = dates,
                           midLengths = ourfish.Masteng.c_miniata.sorted$round_length,
                           catch = as.matrix(ourfish.Masteng.c_miniata.sorted[,-1]))
...
max.length = as.numeric(max(ourfish.Masteng.c_miniata.sorted$round_length))
...
ourfish.Masteng.c_miniata.long <- ourfish.Masteng.c_miniata.sorted %>%
  pivot_longer(
    cols = starts_with("20"),
    names_to = "dates",
    values_to = "value")

ourfish.Masteng.c_miniata.long.2 <- ourfish.Masteng.c_miniata.long %>%
  uncount(weights = value)

#Raw Data
length.raw <- tibble(Length = ourfish.Masteng.c_miniata.long.2$round_length)
...
ggsave("c_miniata size.svg", plot = size.structure.plot, bg = "transparent", width = 13.33, height = 7.3)

#total landings & CPUE L-lentjan================================================
ourfish.c_miniata.landings.1 <- ourfish.Masteng.clean[species %in% "Cephalopholis miniata",
                                            .(sum_landings = sum(weight_kg)),
                                            by = .(date, species)][order(date)]

ourfish.c_miniata.landings.1.glm <- ourfish.c_miniata.landings.1 %>%
  nest(-'species') %>%
  mutate(model = map(data, ~ glm(sum_landings ~ date, data = .))) %>%
  mutate(tidied = map(model, tidy)) %>%
  unnest(tidied)

ourfish.c_miniata.landings.1.glm2 <- ourfish.c_miniata.landings.1.glm %>%
  dplyr::filter(term == "date") %>%
  mutate(p_value_adjusted = p.adjust(p.value)) %>%
  arrange(p_value_adjusted) %>%
  mutate(stat_test = ifelse(p_value_adjusted < 0.05, "significant", "not significant")) %>% #Pvalue <0.05 indicate ada perbedaan antara data dalam kelompok (signifikan)
...
setDT(ourfish.c_miniata.landings.1.glm2)

# Label Landings trends
ourfish.c_miniata.landings.1.glm3 <- ourfish.c_miniata.landings.1.glm2[,
                                                           trend := case_when(estimate >= 0 & stat_test == "significant" ~ "Increased", #estimate is to measure the alternative hypothesis, >=0 means there is positive linear relationship between predictor (date) and response (CPUE or weight)
                                                                              stat_test == "not significant" ~ "Stable",
                                                                              TRUE ~ "Decreased")]

trend.label <- ourfish.c_miniata.landings.1.glm3[, `:=`(term = NULL, estimate = NULL, std.error = NULL, statistic = NULL, p.value = NULL, p_value_adjusted = NULL, stat_test = NULL)]

summary(ourfish.c_miniata.landings.1.glm3)

ourfish.c_miniata.landings.1.plot <- ggplot(ourfish.c_miniata.landings.1, aes(x = date, y = sum_landings)) +
  ...
  geom_text(data = trend.label,
            ...
            #vjust = 5,
            inherit.aes = FALSE) +
  labs(title = "Total Landings c.miniata in MAR Masteng",
       x = "",
       y = "Total landings(kg)") +
  theme.default.1 +
  theme(
...
landings.Masteng.indo <- ourfish.c_miniata.landings.1.plot + theme(
  plot.background = element_rect(fill = "transparent", colour = NA), # Makes the entire plot background transparent
  panel.background = element_rect(fill = "transparent", colour = NA) # Makes the panel background transparent
)
ggsave("landings c_miniata Masteng.svg", plot = landings.Masteng.indo, bg = "transparent", width = 13.33, height = 7.3)


#### CPUE Trends in Southeast Sulawesi ####
# Subset weight data by date, species for total landings
ourfish.c_miniata.cpue.1 <- ourfish.Masteng.clean[species %in% "Cephalopholis miniata",
                                                    .(sum_landings = sum(weight_kg)),
                                                    by = .(date, species, fisher_id)][order(date)]

ourfish.c_miniata.cpue.1.glm <- ourfish.c_miniata.cpue.1 %>%
  nest(-species) %>%
  mutate(model = map(data, ~ glm(sum_landings ~ date, data = .))) %>%
  mutate(tidied = map(model, tidy)) %>%
  unnest(tidied)

ourfish.c_miniata.cpue.1.glm2 <- ourfish.c_miniata.cpue.1.glm %>%
  dplyr::filter(term == "date") %>%
  mutate(p_value_adjusted = p.adjust(p.value)) %>%
  arrange(p_value_adjusted) %>%
  mutate(stat_test = ifelse(p_value_adjusted < 0.05, "significant", "not significant")) %>% #Pvalue <0.05 indicate ada perbedaan antara data dalam kelompok (signifikan)
...
setDT(ourfish.c_miniata.cpue.1.glm2)

# Label Landings trends
ourfish.c_miniata.cpue.1.glm3 <- ourfish.c_miniata.cpue.1.glm2[,
                                                   trend := case_when(estimate >= 0 & stat_test == "significant" ~ "Increased", #estimate is to measure the alternative hypothesis, >=0 means there is positive linear relationship between predictor (date) and response (CPUE or weight)
                                                                      stat_test == "not significant" ~ "Stable",
                                                                      TRUE ~ "Decreased")]

trend.label <- ourfish.c_miniata.cpue.1.glm3[, `:=`(term = NULL, estimate = NULL, std.error = NULL, statistic = NULL, p.value = NULL, p_value_adjusted = NULL, stat_test = NULL)]

summary(ourfish.c_miniata.cpue.1.glm3)

ourfish.c_miniata.cpue.1.plot <- ggplot(ourfish.c_miniata.cpue.1, aes(x = date, y = sum_landings)) +
  geom_smooth(method = "loess", span = 0.5, color = "#B50A37", fill = "#B50A37") +
  scale_y_continuous(expand = c(0,0), limits = c(0,NA), oob = squish) +
  scale_x_date(labels = date_format("%b %Y"),
               date_breaks = "6 month") +
  geom_text(data = trend.label,
            ...
            #vjust = 5,
            inherit.aes = FALSE) +
  labs(title = "Catch Per Unit Effort c.miniata in MAR Masteng",
       x = "",
       y = "CPUE (kg/fisher/trip)") +
  theme.default.1 +
  theme(
...
cpue.Masteng.indo <- ourfish.c_miniata.cpue.1.plot + theme(
  plot.background = element_rect(fill = "transparent", colour = NA), # Makes the entire plot background transparent
  panel.background = element_rect(fill = "transparent", colour = NA) # Makes the panel background transparent
)
ggsave("cpue c_miniata Masteng.svg", plot = cpue.Masteng.indo, bg = "transparent", width = 13.33, height = 7.3)
