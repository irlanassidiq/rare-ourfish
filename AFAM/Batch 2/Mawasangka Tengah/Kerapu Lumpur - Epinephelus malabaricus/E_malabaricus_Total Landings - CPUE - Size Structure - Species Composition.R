...
setwd("C:/Users/IrlanAssidiq/OneDrive - Rare/ourfish/AFAM/Mawasangka Tengah")
getwd()
...
#===============================================================================
# Total Landings
ourfish.Masteng<- ourfish.raw[#month >= 2 & month <= 7 &
  buyer_name %in% "egerman"]

ourfish.ses.landings.Masteng <- ourfish.Masteng[buyer_name %in% "egerman",
                                                  .(Berat = sum(weight_kg)),
                                                  by = .(date, buying_unit, label)][order(date)]

setnames(ourfish.Masteng, "buyer_name", "location")
ourfish.ses.landings.1 <- ourfish.Masteng[location %in% "egerman",
...
ourfish.ses.landings.1.plot <- ggplot(ourfish.ses.landings.1, aes(x = date, y = sum_landings)) +
  ...
  geom_text(data = trend.label,
            ...
            #vjust = 5,
            inherit.aes = FALSE) +
  labs(title = "Total Landings in MAR Masteng",
       x = "",
       y = "Total landings(kg)") +
  theme.default.1 +
  theme(
...
landings.Masteng <- ourfish.ses.landings.1.plot + theme(
  plot.background = element_rect(fill = "transparent", colour = NA), # Makes the entire plot background transparent
  panel.background = element_rect(fill = "transparent", colour = NA) # Makes the panel background transparent
)
ggsave("landings Masteng.svg", plot = landings.Masteng, bg = "transparent", width = 13.33, height = 7.3)


#### CPUE Trends in Southeast Sulawesi ####
# Subset weight data by date, location for total landings
ourfish.ses.cpue.1 <- ourfish.Masteng[location %in% "egerman",
                                       .(sum_landings = sum(weight_kg)),
...
ourfish.ses.cpue.1.plot <- ggplot(ourfish.ses.cpue.1, aes(x = date, y = sum_landings)) +
  ...
  geom_text(data = trend.label,
            ...
            #vjust = 5,
            inherit.aes = FALSE) +
  labs(title = "Catch Per Unit Effort in MAR Masteng",
       x = "",
...
cpue.Masteng <- ourfish.ses.cpue.1.plot + theme(
  plot.background = element_rect(fill = "transparent", colour = NA), # Makes the entire plot background transparent
  panel.background = element_rect(fill = "transparent", colour = NA) # Makes the panel background transparent
)
ggsave("cpue Masteng.svg", plot = cpue.Masteng, bg = "transparent", width = 13.33, height = 7.3)

# Detecting outliers in weight data
library(EnvStats)
quantile(ourfish.Masteng$weight_kg, 0.01)
quantile(ourfish.Masteng$weight_kg, 0.99)

# Number of k value for Rosner Test
num.k.upper.weight <- ourfish.Masteng[weight_kg >= quantile(weight_kg, 0.99), .N]
num.k.lower.weight <- ourfish.Masteng[weight_kg <= quantile(weight_kg, 0.01), .N]
k.value.rosnertest.weight <- num.k.upper.weight + num.k.lower.weight

# Statistical test for outliers using Rosner Test
rosner.test.landings.data <- rosnerTest(ourfish.Masteng$weight_kg, k = k.value.rosnertest.weight)
...
ourfish.Masteng.outlier <- ourfish.Masteng[landings.data.outliers$Obs.Num, ]
...
ourfish.Masteng.clean <- ourfish.Masteng[-landings.data.outliers$Obs.Num, ]

## Box Plot for annual comparison

# Make data for two years period
ourfish.Masteng.clean.boxplot.1 <- ourfish.Masteng.clean[,
                                                     year_period := as.factor("2 Years Period")]

# Make data for Year 1
ourfish.Masteng.clean.boxplot.2 <- ourfish.Masteng.clean[date >= start.date & date <= "2022-10-01"][,
                                                                                                year_period := as.factor("Year 1")]

# Make data for Year 2
ourfish.Masteng.clean.boxplot.3 <- ourfish.Masteng.clean[date >= "2022-08-01" & date <= end.date][,
                                                                                              year_period := as.factor("Year 2")]

# Combine data of two years period with year 1 & 2 data
ourfish.Masteng.clean.boxplot.4 <- rbind(ourfish.Masteng.clean.boxplot.1, ourfish.Masteng.clean.boxplot.2, ourfish.Masteng.clean.boxplot.3)

# Annual CPUE & Catch Value using box plot
ourfish.Masteng.clean.boxplot.5 <- ourfish.Masteng.clean.boxplot.4[,
...
ourfish.Masteng.clean.boxplot.5[,
...
t.test(data = ourfish.Masteng.clean.boxplot.5[!year_period == "2 Years Period"],
...
boxplot.annual.cpue <- 
  ggplot(data = ourfish.Masteng.clean.boxplot.5,
...
ourfish.Masteng.clean.boxplot.6 <- ourfish.Masteng.clean.boxplot.5[,
                                                               .(n_trip = .N,
...
### Average length
ourfish.Masteng.clean$ind_weight_g <- (ourfish.Masteng.clean$weight_kg *1000)/ourfish.Masteng.clean$count
ourfish.Masteng.clean$length <- (ourfish.Masteng.clean$ind_weight_g/ourfish.Masteng.clean$a)^(1/ourfish.Masteng.clean$b)

### average length by family
ourfish.Masteng.prov.ma$ind_weight_g <- (ourfish.Masteng.prov.ma$weight_kg *1000)/ourfish.Masteng.prov.ma$count
ourfish.Masteng.prov.ma$length <- (ourfish.Masteng.prov.ma$ind_weight_g/ourfish.Masteng.prov.ma$a)^(1/ourfish.Masteng.prov.ma$b)


# Subset weight data by date, location, fisher_id, family
ourfish.Masteng.length.fam.1 <- ourfish.Masteng.prov.ma[,
                                                    .(mean_length_fam = mean(length)),
                                                    by = .(date, location, family)][order(date)]

ourfish.Masteng.length.sp.1 <- ourfish.Masteng.prov.ma[,
                                                   .(mean_length_sp = mean(length)),
                                                   by = .(date, location,family, species)][order(date)]

ourfish.Masteng.length.sp.1 <- filter_if(ourfish.Masteng.length.sp.1, is.numeric, all_vars((.) != 0))
table.sp.length <- table(c(ourfish.Masteng.length.sp.1$species,
                           ourfish.Masteng.length.sp.1$mean_length_sp))

# Rename level1_name and ma_name into the same column name to be combined
ourfish.Masteng.combine.1 <- ourfish.Masteng.clean[ , ]
ourfish.Masteng.combine.2 <- ourfish.Masteng.clean[ , ]

# Combine data from whole Masteng with data by MA+R
ourfish.Masteng.prov.ma <- rbind(ourfish.Masteng.combine.1, ourfish.Masteng.combine.2)
...
table.fam <- table(ourfish.Masteng.prov.ma$family)

# Change data into wide format
ourfish.Masteng.length.fam.1.wide <- dcast(ourfish.Masteng.length.fam.1,
                                         formula = date + location ~ family,
                                         value.var = "mean_length_fam")

ourfish.Masteng.length.sp.1.wide <- dcast(ourfish.Masteng.length.sp.1,
                                        formula = date + location + family ~ species,
                                        value.var = "mean_length_sp")

# Replace NA value with 0
ourfish.Masteng.length.fam.1.wide[is.na(ourfish.Masteng.length.fam.1.wide)] <- 0

ourfish.Masteng.length.sp.1.wide[is.na(ourfish.Masteng.length.sp.1.wide)] <- 0

# Change data back into long format
ourfish.Masteng.length.fam.1.long <- melt(ourfish.Masteng.length.fam.1.wide,
                                        measure.vars = c(3:20), # Please check the number of column first
                                        variable.name = "family",
                                        value.name = "mean_length_fam")
ourfish.Masteng.length.fam.1.long.clean <- filter_if(ourfish.Masteng.length.fam.1.long, is.numeric, all_vars((.) !=0))
ourfish.Masteng.length.fam.1.long.clean <- filter_if(ourfish.Masteng.length.fam.1.long.clean, is.numeric, all_vars((.) !=Inf))
ourfish.Masteng.length.sp.1.long <- melt(ourfish.Masteng.length.sp.1.wide,
                                       measure.vars = c(4:45), # Please check the number of column first
                                       variable.name = "species",
                                       value.name = "mean_length_sp")

ourfish.Masteng.length.sp.1.long.clean <- filter_if(ourfish.Masteng.length.sp.1.long, is.numeric, all_vars((.) != 0))
ourfish.Masteng.length.sp.1.long.clean <- filter_if(ourfish.Masteng.length.sp.1.long.clean, is.numeric, all_vars((.) !=Inf)) 

table.sp.clean <- table(ourfish.Masteng.length.sp.1.long.clean$species)

#Size Structure
#creating new column for Year_month
library(zoo)
ourfish.Masteng.clean$year_month <- ourfish.Masteng.clean$date
ourfish.Masteng.clean$year_month <- as.yearmon(ourfish.Masteng.clean$year_month, format = "%Y_%m")

#creating midlength, using round up function
ourfish.Masteng.clean$round_length <- round(ourfish.Masteng.clean$length)

#Katamba - Epinephelus malabaricus
ourfish.Masteng.E_malabaricus <- ourfish.Masteng.clean[species%in% "Epinephelus malabaricus",
                                                         .(count = sum(count)),
                                                         by = .(year_month, round_length)] [order(year_month)]
ourfish.Masteng.E_malabaricus$year_month <- as.Date(ourfish.Masteng.E_malabaricus$year_month)
ourfish.Masteng.E_malabaricus.wide <- ourfish.Masteng.E_malabaricus %>%
  pivot_wider(names_from = year_month,
              values_from = count,
              values_fill = 0)

ourfish.Masteng.E_malabaricus.sorted <- ourfish.Masteng.E_malabaricus.wide %>% arrange(round_length)

#===============================================================================
#Data arrangement: agar bisa dibaca sama R
dates <- colnames(ourfish.Masteng.E_malabaricus.sorted)[-1]
...
length.series.data <- list(dates = dates,
                           midLengths = ourfish.Masteng.E_malabaricus.sorted$round_length,
                           catch = as.matrix(ourfish.Masteng.E_malabaricus.sorted[,-1]))
...
max.length = as.numeric(max(ourfish.Masteng.E_malabaricus.sorted$round_length))
...
K.fishlife = 0.101 #Input K value from Fish Life script or other literatures/sources
...
length.series.data$M = 0.123

#preparing raw data for length series
ourfish.Masteng.E_malabaricus.long <- ourfish.Masteng.E_malabaricus.sorted %>%
  pivot_longer(
    cols = starts_with("20"),
    names_to = "dates",
    values_to = "value")

ourfish.Masteng.E_malabaricus.long.2 <- ourfish.Masteng.E_malabaricus.long %>%
  uncount(weights = value)

#Raw Data
length.raw <- tibble(Length = ourfish.Masteng.E_malabaricus.long.2$round_length)
...
ggsave("E_malabaricus size.svg", plot = size.structure.plot, bg = "transparent", width = 13.33, height = 7.3)

#total landings & CPUE L-lentjan================================================
ourfish.E_malabaricus.landings.1 <- ourfish.Masteng.clean[species %in% "Epinephelus malabaricus",
                                            .(sum_landings = sum(weight_kg)),
                                            by = .(date, species)][order(date)]

ourfish.E_malabaricus.landings.1.glm <- ourfish.E_malabaricus.landings.1 %>%
  nest(-'species') %>%
  mutate(model = map(data, ~ glm(sum_landings ~ date, data = .))) %>%
  mutate(tidied = map(model, tidy)) %>%
  unnest(tidied)

ourfish.E_malabaricus.landings.1.glm2 <- ourfish.E_malabaricus.landings.1.glm %>%
  dplyr::filter(term == "date") %>%
  mutate(p_value_adjusted = p.adjust(p.value)) %>%
  arrange(p_value_adjusted) %>%
  mutate(stat_test = ifelse(p_value_adjusted < 0.05, "significant", "not significant")) %>% #Pvalue <0.05 indicate ada perbedaan antara data dalam kelompok (signifikan)
...
setDT(ourfish.E_malabaricus.landings.1.glm2)

# Label Landings trends
ourfish.E_malabaricus.landings.1.glm3 <- ourfish.E_malabaricus.landings.1.glm2[,
                                                           trend := case_when(estimate >= 0 & stat_test == "significant" ~ "Increased", #estimate is to measure the alternative hypothesis, >=0 means there is positive linear relationship between predictor (date) and response (CPUE or weight)
                                                                              stat_test == "not significant" ~ "Stable",
                                                                              TRUE ~ "Decreased")]

trend.label <- ourfish.E_malabaricus.landings.1.glm3[, `:=`(term = NULL, estimate = NULL, std.error = NULL, statistic = NULL, p.value = NULL, p_value_adjusted = NULL, stat_test = NULL)]

summary(ourfish.E_malabaricus.landings.1.glm3)

ourfish.E_malabaricus.landings.1.plot <- ggplot(ourfish.E_malabaricus.landings.1, aes(x = date, y = sum_landings)) +
  ...
  geom_text(data = trend.label,
            ...
            #vjust = 5,
            inherit.aes = FALSE) +
  labs(title = "Total Landings E.malabaricus in MAR Masteng",
       x = "",
       y = "Total landings(kg)") +
  theme.default.1 +
  theme(
...
landings.Masteng.indo <- ourfish.E_malabaricus.landings.1.plot + theme(
  plot.background = element_rect(fill = "transparent", colour = NA), # Makes the entire plot background transparent
  panel.background = element_rect(fill = "transparent", colour = NA) # Makes the panel background transparent
)
ggsave("landings E_malabaricus Masteng.svg", plot = landings.Masteng.indo, bg = "transparent", width = 13.33, height = 7.3)


#### CPUE Trends in Southeast Sulawesi ####
# Subset weight data by date, species for total landings
ourfish.E_malabaricus.cpue.1 <- ourfish.Masteng.clean[species %in% "Epinephelus malabaricus",
                                                    .(sum_landings = sum(weight_kg)),
                                                    by = .(date, species, fisher_id)][order(date)]

ourfish.E_malabaricus.cpue.1.glm <- ourfish.E_malabaricus.cpue.1 %>%
  nest(-species) %>%
  mutate(model = map(data, ~ glm(sum_landings ~ date, data = .))) %>%
  mutate(tidied = map(model, tidy)) %>%
  unnest(tidied)

ourfish.E_malabaricus.cpue.1.glm2 <- ourfish.E_malabaricus.cpue.1.glm %>%
  dplyr::filter(term == "date") %>%
  mutate(p_value_adjusted = p.adjust(p.value)) %>%
  arrange(p_value_adjusted) %>%
  mutate(stat_test = ifelse(p_value_adjusted < 0.05, "significant", "not significant")) %>% #Pvalue <0.05 indicate ada perbedaan antara data dalam kelompok (signifikan)
...
setDT(ourfish.E_malabaricus.cpue.1.glm2)

# Label Landings trends
ourfish.E_malabaricus.cpue.1.glm3 <- ourfish.E_malabaricus.cpue.1.glm2[,
                                                   trend := case_when(estimate >= 0 & stat_test == "significant" ~ "Increased", #estimate is to measure the alternative hypothesis, >=0 means there is positive linear relationship between predictor (date) and response (CPUE or weight)
                                                                      stat_test == "not significant" ~ "Stable",
                                                                      TRUE ~ "Decreased")]

trend.label <- ourfish.E_malabaricus.cpue.1.glm3[, `:=`(term = NULL, estimate = NULL, std.error = NULL, statistic = NULL, p.value = NULL, p_value_adjusted = NULL, stat_test = NULL)]

summary(ourfish.E_malabaricus.cpue.1.glm3)

ourfish.E_malabaricus.cpue.1.plot <- ggplot(ourfish.E_malabaricus.cpue.1, aes(x = date, y = sum_landings)) +
  geom_smooth(method = "loess", span = 0.5, color = "#B50A37", fill = "#B50A37") +
  scale_y_continuous(expand = c(0,0), limits = c(0,NA), oob = squish) +
  scale_x_date(labels = date_format("%b %Y"),
               date_breaks = "6 month") +
  geom_text(data = trend.label,
            ...
            #vjust = 5,
            inherit.aes = FALSE) +
  labs(title = "Catch Per Unit Effort E.malabaricus in MAR Masteng",
       x = "",
       y = "CPUE (kg/fisher/trip)") +
  theme.default.1 +
  theme(
...
cpue.Masteng.indo <- ourfish.E_malabaricus.cpue.1.plot + theme(
  plot.background = element_rect(fill = "transparent", colour = NA), # Makes the entire plot background transparent
  panel.background = element_rect(fill = "transparent", colour = NA) # Makes the panel background transparent
)
ggsave("cpue E_malabaricus Masteng.svg", plot = cpue.Masteng.indo, bg = "transparent", width = 13.33, height = 7.3)
