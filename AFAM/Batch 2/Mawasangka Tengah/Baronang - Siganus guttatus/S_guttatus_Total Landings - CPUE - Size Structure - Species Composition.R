...
ourfish.Masteng.s_guttatus <- ourfish.Masteng.clean[species%in% "Siganus guttatus",
                                                         .(count = sum(count)),
                                                         by = .(year_month, round_length)] [order(year_month)]
ourfish.Masteng.s_guttatus$year_month <- as.Date(ourfish.Masteng.s_guttatus$year_month)
ourfish.Masteng.s_guttatus.wide <- ourfish.Masteng.s_guttatus %>%
  pivot_wider(names_from = year_month,
              values_from = count,
              values_fill = 0)

ourfish.Masteng.s_guttatus.sorted <- ourfish.Masteng.s_guttatus.wide %>% arrange(round_length)

#===============================================================================
#Data arrangement: agar bisa dibaca sama R
dates <- colnames(ourfish.Masteng.s_guttatus.sorted)[-1]
...
length.series.data <- list(dates = dates,
                           midLengths = ourfish.Masteng.s_guttatus.sorted$round_length,
                           catch = as.matrix(ourfish.Masteng.s_guttatus.sorted[,-1]))
...
max.length = as.numeric(max(ourfish.Masteng.s_guttatus.sorted$round_length))
...
length.series.data$M = 1.435

#preparing raw data for length series
ourfish.Masteng.s_guttatus.long <- ourfish.Masteng.s_guttatus.sorted %>%
  pivot_longer(
    cols = starts_with("20"),
    names_to = "dates",
    values_to = "value")

ourfish.Masteng.s_guttatus.long.2 <- ourfish.Masteng.s_guttatus.long %>%
  uncount(weights = value)

#Raw Data
length.raw <- tibble(Length = ourfish.Masteng.s_guttatus.long.2$round_length)
...
ourfish.s_guttatus.landings.1 <- ourfish.Masteng.clean[species %in% "Siganus guttatus",
                                            .(sum_landings = sum(weight_kg)),
...
ourfish.s_guttatus.landings.1.plot <- ggplot(ourfish.s_guttatus.landings.1, aes(x = date, y = sum_landings)) +
  ...
  geom_text(data = trend.label,
            ...
            #vjust = 5,
            inherit.aes = FALSE) +
  labs(title = "Total Landings S.guttatus in MAR Masteng",
       x = "",
       y = "Total landings(kg)") +
  theme.default.1 +
  theme(
...
landings.Masteng.indo <- ourfish.s_guttatus.landings.1.plot + theme(
  plot.background = element_rect(fill = "transparent", colour = NA), # Makes the entire plot background transparent
  panel.background = element_rect(fill = "transparent", colour = NA) # Makes the panel background transparent
)
ggsave("landings s_guttatus Masteng.svg", plot = landings.Masteng.indo, bg = "transparent", width = 13.33, height = 7.3)


#### CPUE Trends in Southeast Sulawesi ####
# Subset weight data by date, species for total landings
ourfish.s_guttatus.cpue.1 <- ourfish.Masteng.clean[species %in% "Siganus guttatus",
                                                    .(sum_landings = sum(weight_kg)),
...
ourfish.s_guttatus.cpue.1.plot <- ggplot(ourfish.s_guttatus.cpue.1, aes(x = date, y = sum_landings)) +
  ...
  geom_text(data = trend.label,
            ...
            #vjust = 5,
            inherit.aes = FALSE) +
  labs(title = "Catch Per Unit Effort S.guttatus in MAR Masteng",
       x = "",
       y = "CPUE (kg/fisher/trip)") +
  theme.default.1 +
  theme(
...
cpue.Masteng.indo <- ourfish.s_guttatus.cpue.1.plot + theme(
  plot.background = element_rect(fill = "transparent", colour = NA), # Makes the entire plot background transparent
  panel.background = element_rect(fill = "transparent", colour = NA) # Makes the panel background transparent
)
ggsave("cpue s_guttatus Masteng.svg", plot = cpue.Masteng.indo, bg = "transparent", width = 13.33, height = 7.3)
