...
#Katamba - Epinephelus coioides
ourfish.Mawasangka.e_coioides <- ourfish.Mawasangka.clean[species%in% "Epinephelus coioides",
                                                         .(count = sum(count)),
                                                         by = .(year_month, round_length)] [order(year_month)]
ourfish.Mawasangka.e_coioides$year_month <- as.Date(ourfish.Mawasangka.e_coioides$year_month)
ourfish.Mawasangka.e_coioides.wide <- ourfish.Mawasangka.e_coioides %>%
  pivot_wider(names_from = year_month,
              values_from = count,
              values_fill = 0)

ourfish.Mawasangka.e_coioides.sorted <- ourfish.Mawasangka.e_coioides.wide %>% arrange(round_length)

#===============================================================================
#Data arrangement: agar bisa dibaca sama R
dates <- colnames(ourfish.Mawasangka.e_coioides.sorted)[-1]
...
length.series.data <- list(dates = dates,
                           midLengths = ourfish.Mawasangka.e_coioides.sorted$round_length,
                           catch = as.matrix(ourfish.Mawasangka.e_coioides.sorted[,-1]))
...
max.length = as.numeric(max(ourfish.Mawasangka.e_coioides.sorted$round_length))
...
K.fishlife = 0.14 #Input K value from Fish Life script or other literatures/sources
...
length.series.data$M = 0.174

#preparing raw data for length series
ourfish.Mawasangka.e_coioides.long <- ourfish.Mawasangka.e_coioides.sorted %>%
  pivot_longer(
    cols = starts_with("20"),
    names_to = "dates",
    values_to = "value")

ourfish.Mawasangka.e_coioides.long.2 <- ourfish.Mawasangka.e_coioides.long %>%
  uncount(weights = value)

#Raw Data
length.raw <- tibble(Length = ourfish.Mawasangka.e_coioides.long.2$round_length)
...
ggsave("e_coioides size.svg", plot = size.structure.plot, bg = "transparent", width = 13.33, height = 7.3)

#total landings & CPUE L-lentjan================================================
ourfish.e_coioides.landings.1 <- ourfish.Mawasangka.clean[species %in% "Epinephelus coioides",
                                            .(sum_landings = sum(weight_kg)),
                                            by = .(date, species)][order(date)]

ourfish.e_coioides.landings.1.glm <- ourfish.e_coioides.landings.1 %>%
  nest(-'species') %>%
  mutate(model = map(data, ~ glm(sum_landings ~ date, data = .))) %>%
  mutate(tidied = map(model, tidy)) %>%
  unnest(tidied)

ourfish.e_coioides.landings.1.glm2 <- ourfish.e_coioides.landings.1.glm %>%
  dplyr::filter(term == "date") %>%
  mutate(p_value_adjusted = p.adjust(p.value)) %>%
  arrange(p_value_adjusted) %>%
  mutate(stat_test = ifelse(p_value_adjusted < 0.05, "significant", "not significant")) %>% #Pvalue <0.05 indicate ada perbedaan antara data dalam kelompok (signifikan)
...
setDT(ourfish.e_coioides.landings.1.glm2)

# Label Landings trends
ourfish.e_coioides.landings.1.glm3 <- ourfish.e_coioides.landings.1.glm2[,
                                                           trend := case_when(estimate >= 0 & stat_test == "significant" ~ "Increased", #estimate is to measure the alternative hypothesis, >=0 means there is positive linear relationship between predictor (date) and response (CPUE or weight)
                                                                              stat_test == "not significant" ~ "Stable",
                                                                              TRUE ~ "Decreased")]

trend.label <- ourfish.e_coioides.landings.1.glm3[, `:=`(term = NULL, estimate = NULL, std.error = NULL, statistic = NULL, p.value = NULL, p_value_adjusted = NULL, stat_test = NULL)]

summary(ourfish.e_coioides.landings.1.glm3)

ourfish.e_coioides.landings.1.plot <- ggplot(ourfish.e_coioides.landings.1, aes(x = date, y = sum_landings)) +
  ...
  geom_text(data = trend.label,
            ...
            #vjust = 5,
            inherit.aes = FALSE) +
  labs(title = "Total Landings E.coioides in MAR Mawasangka",
       x = "",
       y = "Total landings(kg)") +
  theme.default.1 +
  theme(
...
landings.Mawasangka.indo <- ourfish.e_coioides.landings.1.plot + theme(
  plot.background = element_rect(fill = "transparent", colour = NA), # Makes the entire plot background transparent
  panel.background = element_rect(fill = "transparent", colour = NA) # Makes the panel background transparent
)
ggsave("landings e_coioides Mawasangka.svg", plot = landings.Mawasangka.indo, bg = "transparent", width = 13.33, height = 7.3)


#### CPUE Trends in Southeast Sulawesi ####
# Subset weight data by date, species for total landings
ourfish.e_coioides.cpue.1 <- ourfish.Mawasangka.clean[species %in% "Epinephelus coioides",
                                                    .(sum_landings = sum(weight_kg)),
                                                    by = .(date, species, fisher_id)][order(date)]

ourfish.e_coioides.cpue.1.glm <- ourfish.e_coioides.cpue.1 %>%
  nest(-species) %>%
  mutate(model = map(data, ~ glm(sum_landings ~ date, data = .))) %>%
  mutate(tidied = map(model, tidy)) %>%
  unnest(tidied)

ourfish.e_coioides.cpue.1.glm2 <- ourfish.e_coioides.cpue.1.glm %>%
  dplyr::filter(term == "date") %>%
  mutate(p_value_adjusted = p.adjust(p.value)) %>%
  arrange(p_value_adjusted) %>%
  mutate(stat_test = ifelse(p_value_adjusted < 0.05, "significant", "not significant")) %>% #Pvalue <0.05 indicate ada perbedaan antara data dalam kelompok (signifikan)
...
setDT(ourfish.e_coioides.cpue.1.glm2)

# Label Landings trends
ourfish.e_coioides.cpue.1.glm3 <- ourfish.e_coioides.cpue.1.glm2[,
                                                   trend := case_when(estimate >= 0 & stat_test == "significant" ~ "Increased", #estimate is to measure the alternative hypothesis, >=0 means there is positive linear relationship between predictor (date) and response (CPUE or weight)
                                                                      stat_test == "not significant" ~ "Stable",
                                                                      TRUE ~ "Decreased")]

trend.label <- ourfish.e_coioides.cpue.1.glm3[, `:=`(term = NULL, estimate = NULL, std.error = NULL, statistic = NULL, p.value = NULL, p_value_adjusted = NULL, stat_test = NULL)]

summary(ourfish.e_coioides.cpue.1.glm3)

ourfish.e_coioides.cpue.1.plot <- ggplot(ourfish.e_coioides.cpue.1, aes(x = date, y = sum_landings)) +
  geom_smooth(method = "loess", span = 0.5, color = "#e78828", fill = "#e78828") +
  scale_y_continuous(expand = c(0,0), limits = c(0,NA), oob = squish) +
  scale_x_date(labels = date_format("%b %Y"),
               date_breaks = "6 month") +
  geom_text(data = trend.label,
            ...
            #vjust = 5,
            inherit.aes = FALSE) +
  labs(title = "Catch Per Unit Effort E.coioides in MAR Mawasangka",
       x = "",
       y = "CPUE (kg/fisher/trip)") +
  theme.default.1 +
  theme(
...
cpue.Mawasangka.indo <- ourfish.e_coioides.cpue.1.plot + theme(
  plot.background = element_rect(fill = "transparent", colour = NA), # Makes the entire plot background transparent
  panel.background = element_rect(fill = "transparent", colour = NA) # Makes the panel background transparent
)
ggsave("cpue e_coioides Mawasangka.svg", plot = cpue.Mawasangka.indo, bg = "transparent", width = 13.33, height = 7.3)
