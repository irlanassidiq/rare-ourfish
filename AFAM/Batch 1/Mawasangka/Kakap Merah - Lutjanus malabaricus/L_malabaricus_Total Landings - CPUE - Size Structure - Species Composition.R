...
setwd("C:/Users/IrlanAssidiq/OneDrive - Rare/ourfish/AFAM/Mawasangka")
getwd()
...
#===============================================================================
# Total Landings
ourfish.Mawasangka<- ourfish.raw[#month >= 2 & month <= 7 &
  ma_name %in% "Mawasangka"]

ourfish.ses.landings.Mawasangka <- ourfish.Mawasangka[ma_name %in% "Mawasangka",
                                                  .(Berat = sum(weight_kg)),
                                                  by = .(date, buying_unit, label)][order(date)]

setnames(ourfish.Mawasangka, "ma_name", "location")
ourfish.ses.landings.1 <- ourfish.Mawasangka[location %in% "Mawasangka",
...
ourfish.ses.landings.1.plot <- ggplot(ourfish.ses.landings.1, aes(x = date, y = sum_landings)) +
  geom_smooth(method = "loess", span = 0.5, color = "#B50A37", fill = "#B50A37") +
  ...
  geom_text(data = trend.label,
            aes(x = as.Date("2022-06-01", format("%Y-%m-%d")),
                ...
                label = trend),
            hjust = 0.1,
            color = "#B50A37", #Orange #e78828; Hijau #00833e; Tosca #06a7b4
            size = 6,
            #vjust = 5,
            inherit.aes = FALSE) +
...
landings.Mawasangka <- ourfish.ses.landings.1.plot + theme(
  plot.background = element_rect(fill = "transparent", colour = NA), # Makes the entire plot background transparent
  panel.background = element_rect(fill = "transparent", colour = NA) # Makes the panel background transparent
)
ggsave("landings Mawasangka.svg", plot = landings.Mawasangka, bg = "transparent", width = 13.33, height = 7.3)


#### CPUE Trends in Southeast Sulawesi ####
# Subset weight data by date, location for total landings
ourfish.ses.cpue.1 <- ourfish.Mawasangka[location %in% "Mawasangka",
                                       .(sum_landings = sum(weight_kg)),
...
ourfish.ses.cpue.1.plot <- ggplot(ourfish.ses.cpue.1, aes(x = date, y = sum_landings)) +
  ...
  geom_text(data = trend.label,
            aes(x = as.Date("2022-06-01", format("%Y-%m-%d")),
                ...
                label = trend),
            hjust = 0.1,
            color = "#00833e",  #Orange #e78828; Hijau #00833e; Tosca #06a7b4; Merah #B50A37
            size = 6,
            #vjust = 5,
            inherit.aes = FALSE) +
...
cpue.Mawasangka <- ourfish.ses.cpue.1.plot + theme(
  plot.background = element_rect(fill = "transparent", colour = NA), # Makes the entire plot background transparent
  panel.background = element_rect(fill = "transparent", colour = NA) # Makes the panel background transparent
)
ggsave("cpue Mawasangka.svg", plot = cpue.Mawasangka, bg = "transparent", width = 13.33, height = 7.3)

# Detecting outliers in weight data
library(EnvStats)
quantile(ourfish.Mawasangka$weight_kg, 0.01)
quantile(ourfish.Mawasangka$weight_kg, 0.99)

# Number of k value for Rosner Test
num.k.upper.weight <- ourfish.Mawasangka[weight_kg >= quantile(weight_kg, 0.99), .N]
num.k.lower.weight <- ourfish.Mawasangka[weight_kg <= quantile(weight_kg, 0.01), .N]
k.value.rosnertest.weight <- num.k.upper.weight + num.k.lower.weight

# Statistical test for outliers using Rosner Test
rosner.test.landings.data <- rosnerTest(ourfish.Mawasangka$weight_kg, k = k.value.rosnertest.weight)
...
ourfish.Mawasangka.outlier <- ourfish.Mawasangka[landings.data.outliers$Obs.Num, ]
...
ourfish.Mawasangka.clean <- ourfish.Mawasangka[-landings.data.outliers$Obs.Num, ]

## Box Plot for annual comparison

# Make data for two years period
ourfish.Mawasangka.clean.boxplot.1 <- ourfish.Mawasangka.clean[,
                                                     year_period := as.factor("2 Years Period")]

# Make data for Year 1
ourfish.Mawasangka.clean.boxplot.2 <- ourfish.Mawasangka.clean[date >= start.date & date <= "2022-10-01"][,
                                                                                                year_period := as.factor("Year 1")]

# Make data for Year 2
ourfish.Mawasangka.clean.boxplot.3 <- ourfish.Mawasangka.clean[date >= "2022-08-01" & date <= end.date][,
                                                                                              year_period := as.factor("Year 2")]

# Combine data of two years period with year 1 & 2 data
ourfish.Mawasangka.clean.boxplot.4 <- rbind(ourfish.Mawasangka.clean.boxplot.1, ourfish.Mawasangka.clean.boxplot.2, ourfish.Mawasangka.clean.boxplot.3)

# Annual CPUE & Catch Value using box plot
ourfish.Mawasangka.clean.boxplot.5 <- ourfish.Mawasangka.clean.boxplot.4[,
...
ourfish.Mawasangka.clean.boxplot.5[,
...
t.test(data = ourfish.Mawasangka.clean.boxplot.5[!year_period == "2 Years Period"],
...
boxplot.annual.cpue <- 
  ggplot(data = ourfish.Mawasangka.clean.boxplot.5,
...
ourfish.Mawasangka.clean.boxplot.6 <- ourfish.Mawasangka.clean.boxplot.5[,
                                                               .(n_trip = .N,
...
### Average length
ourfish.Mawasangka.clean$ind_weight_g <- (ourfish.Mawasangka.clean$weight_kg *1000)/ourfish.Mawasangka.clean$count
ourfish.Mawasangka.clean$length <- (ourfish.Mawasangka.clean$ind_weight_g/ourfish.Mawasangka.clean$a)^(1/ourfish.Mawasangka.clean$b)

### average length by family
ourfish.Mawasangka.prov.ma$ind_weight_g <- (ourfish.Mawasangka.prov.ma$weight_kg *1000)/ourfish.Mawasangka.prov.ma$count
ourfish.Mawasangka.prov.ma$length <- (ourfish.Mawasangka.prov.ma$ind_weight_g/ourfish.Mawasangka.prov.ma$a)^(1/ourfish.Mawasangka.prov.ma$b)


# Subset weight data by date, location, fisher_id, family
ourfish.Mawasangka.length.fam.1 <- ourfish.Mawasangka.prov.ma[,
                                                    .(mean_length_fam = mean(length)),
                                                    by = .(date, location, family)][order(date)]

ourfish.Mawasangka.length.sp.1 <- ourfish.Mawasangka.prov.ma[,
                                                   .(mean_length_sp = mean(length)),
                                                   by = .(date, location,family, species)][order(date)]

ourfish.Mawasangka.length.sp.1 <- filter_if(ourfish.Mawasangka.length.sp.1, is.numeric, all_vars((.) != 0))
table.sp.length <- table(c(ourfish.Mawasangka.length.sp.1$species,
                           ourfish.Mawasangka.length.sp.1$mean_length_sp))

# Rename level1_name and ma_name into the same column name to be combined
ourfish.Mawasangka.combine.1 <- ourfish.Mawasangka.clean[ , ]
ourfish.Mawasangka.combine.2 <- ourfish.Mawasangka.clean[ , ]

# Combine data from whole Mawasangka with data by MA+R
ourfish.Mawasangka.prov.ma <- rbind(ourfish.Mawasangka.combine.1, ourfish.Mawasangka.combine.2)
...
table.fam <- table(ourfish.Mawasangka.prov.ma$family)

# Change data into wide format
ourfish.Mawasangka.length.fam.1.wide <- dcast(ourfish.Mawasangka.length.fam.1,
                                         formula = date + location ~ family,
                                         value.var = "mean_length_fam")

ourfish.Mawasangka.length.sp.1.wide <- dcast(ourfish.Mawasangka.length.sp.1,
                                        formula = date + location + family ~ species,
                                        value.var = "mean_length_sp")

# Replace NA value with 0
ourfish.Mawasangka.length.fam.1.wide[is.na(ourfish.Mawasangka.length.fam.1.wide)] <- 0

ourfish.Mawasangka.length.sp.1.wide[is.na(ourfish.Mawasangka.length.sp.1.wide)] <- 0

# Change data back into long format
ourfish.Mawasangka.length.fam.1.long <- melt(ourfish.Mawasangka.length.fam.1.wide,
                                        measure.vars = c(3:20), # Please check the number of column first
                                        variable.name = "family",
                                        value.name = "mean_length_fam")
ourfish.Mawasangka.length.fam.1.long.clean <- filter_if(ourfish.Mawasangka.length.fam.1.long, is.numeric, all_vars((.) !=0))
ourfish.Mawasangka.length.fam.1.long.clean <- filter_if(ourfish.Mawasangka.length.fam.1.long.clean, is.numeric, all_vars((.) !=Inf))
ourfish.Mawasangka.length.sp.1.long <- melt(ourfish.Mawasangka.length.sp.1.wide,
                                       measure.vars = c(4:45), # Please check the number of column first
                                       variable.name = "species",
                                       value.name = "mean_length_sp")

ourfish.Mawasangka.length.sp.1.long.clean <- filter_if(ourfish.Mawasangka.length.sp.1.long, is.numeric, all_vars((.) != 0))
ourfish.Mawasangka.length.sp.1.long.clean <- filter_if(ourfish.Mawasangka.length.sp.1.long.clean, is.numeric, all_vars((.) !=Inf)) 

table.sp.clean <- table(ourfish.Mawasangka.length.sp.1.long.clean$species)

#Size Structure
#creating new column for Year_month
library(zoo)
ourfish.Mawasangka.clean$year_month <- ourfish.Mawasangka.clean$date
ourfish.Mawasangka.clean$year_month <- as.yearmon(ourfish.Mawasangka.clean$year_month, format = "%Y_%m")

#creating midlength, using round up function
ourfish.Mawasangka.clean$round_length <- round(ourfish.Mawasangka.clean$length)

#Katamba - Lutjanus malabaricus
ourfish.Mawasangka.l_malabaricus <- ourfish.Mawasangka.clean[species%in% "Lutjanus malabaricus",
                                                         .(count = sum(count)),
                                                         by = .(year_month, round_length)] [order(year_month)]
ourfish.Mawasangka.l_malabaricus$year_month <- as.Date(ourfish.Mawasangka.l_malabaricus$year_month)
ourfish.Mawasangka.l_malabaricus.wide <- ourfish.Mawasangka.l_malabaricus %>%
  pivot_wider(names_from = year_month,
              values_from = count,
              values_fill = 0)

ourfish.Mawasangka.l_malabaricus.sorted <- ourfish.Mawasangka.l_malabaricus.wide %>% arrange(round_length)

#===============================================================================
#Data arrangement: agar bisa dibaca sama R
dates <- colnames(ourfish.Mawasangka.l_malabaricus.sorted)[-1]
...
length.series.data <- list(dates = dates,
                           midLengths = ourfish.Mawasangka.l_malabaricus.sorted$round_length,
                           catch = as.matrix(ourfish.Mawasangka.l_malabaricus.sorted[,-1]))
...
max.length = as.numeric(max(ourfish.Mawasangka.l_malabaricus.sorted$round_length))
...
K.fishlife = 0.72 #Input K value from Fish Life script or other literatures/sources
...
length.series.data$M = 0.847

#preparing raw data for length series
ourfish.Mawasangka.l_malabaricus.long <- ourfish.Mawasangka.l_malabaricus.sorted %>%
  pivot_longer(
    cols = starts_with("20"),
    names_to = "dates",
    values_to = "value")

ourfish.Mawasangka.l_malabaricus.long.2 <- ourfish.Mawasangka.l_malabaricus.long %>%
  uncount(weights = value)

#Raw Data
length.raw <- tibble(Length = ourfish.Mawasangka.l_malabaricus.long.2$round_length)
...
percentOpt = round(length(which(length.raw$Length > Lopt.lower)) / length(length.raw$Length) * 100, digits = 1)
...
size.structure.plot <- ggplot(size.structure, aes(x=Length, y = Freq)) +
  ...
  geom_label(x = Lmat, y = -1, 
           label = paste("PMat =", percentMature, "%", sep = " "),
           fill = "white"
           )+
  geom_vline(xintercept = c(Lopt.lower, Lopt.upper),
             ...
             size = 2,
             color = "#F58232")+
  geom_label(x = Lopt.lower, y = -1, 
             label = paste("POpt =", percentOpt, "%", sep = " "),
             fill = "white"
  )+
  ...
  geom_label(x = Lmega, y = -1, 
             label = paste("PMega =", percentMega, "%", sep = " "),
             fill = "white"
  )+
  theme.default.1 +
  theme(
...
ggsave("l_malabaricus size.svg", plot = size.structure.plot, bg = "transparent", width = 13.33, height = 7.3)

#total landings & CPUE L-lentjan================================================
ourfish.l_malabaricus.landings.1 <- ourfish.Mawasangka.clean[species %in% "Lutjanus malabaricus",
                                            .(sum_landings = sum(weight_kg)),
                                            by = .(date, species)][order(date)]

ourfish.l_malabaricus.landings.1.glm <- ourfish.l_malabaricus.landings.1 %>%
  nest(-'species') %>%
  mutate(model = map(data, ~ glm(sum_landings ~ date, data = .))) %>%
  mutate(tidied = map(model, tidy)) %>%
  unnest(tidied)

ourfish.l_malabaricus.landings.1.glm2 <- ourfish.l_malabaricus.landings.1.glm %>%
  dplyr::filter(term == "date") %>%
  mutate(p_value_adjusted = p.adjust(p.value)) %>%
  arrange(p_value_adjusted) %>%
  mutate(stat_test = ifelse(p_value_adjusted < 0.05, "significant", "not significant")) %>% #Pvalue <0.05 indicate ada perbedaan antara data dalam kelompok (signifikan)
...
setDT(ourfish.l_malabaricus.landings.1.glm2)

# Label Landings trends
ourfish.l_malabaricus.landings.1.glm3 <- ourfish.l_malabaricus.landings.1.glm2[,
                                                           trend := case_when(estimate >= 0 & stat_test == "significant" ~ "Increased", #estimate is to measure the alternative hypothesis, >=0 means there is positive linear relationship between predictor (date) and response (CPUE or weight)
                                                                              stat_test == "not significant" ~ "Stable",
                                                                              TRUE ~ "Decreased")]

trend.label <- ourfish.l_malabaricus.landings.1.glm3[, `:=`(term = NULL, estimate = NULL, std.error = NULL, statistic = NULL, p.value = NULL, p_value_adjusted = NULL, stat_test = NULL)]

summary(ourfish.l_malabaricus.landings.1.glm3)

ourfish.l_malabaricus.landings.1.plot <- ggplot(ourfish.l_malabaricus.landings.1, aes(x = date, y = sum_landings)) +
  ...
  geom_text(data = trend.label,
            ...
            #vjust = 5,
            inherit.aes = FALSE) +
  labs(title = "Total Landings L.malabaricus in MAR Mawasangka",
       x = "",
       y = "Total landings(kg)") +
  theme.default.1 +
  theme(
...
landings.Mawasangka.indo <- ourfish.l_malabaricus.landings.1.plot + theme(
  plot.background = element_rect(fill = "transparent", colour = NA), # Makes the entire plot background transparent
  panel.background = element_rect(fill = "transparent", colour = NA) # Makes the panel background transparent
)
ggsave("landings l_malabaricus Mawasangka.svg", plot = landings.Mawasangka.indo, bg = "transparent", width = 13.33, height = 7.3)


#### CPUE Trends in Southeast Sulawesi ####
# Subset weight data by date, species for total landings
ourfish.l_malabaricus.cpue.1 <- ourfish.Mawasangka.clean[species %in% "Lutjanus malabaricus",
                                                    .(sum_landings = sum(weight_kg)),
                                                    by = .(date, species, fisher_id)][order(date)]

ourfish.l_malabaricus.cpue.1.glm <- ourfish.l_malabaricus.cpue.1 %>%
  nest(-species) %>%
  mutate(model = map(data, ~ glm(sum_landings ~ date, data = .))) %>%
  mutate(tidied = map(model, tidy)) %>%
  unnest(tidied)

ourfish.l_malabaricus.cpue.1.glm2 <- ourfish.l_malabaricus.cpue.1.glm %>%
  dplyr::filter(term == "date") %>%
  mutate(p_value_adjusted = p.adjust(p.value)) %>%
  arrange(p_value_adjusted) %>%
  mutate(stat_test = ifelse(p_value_adjusted < 0.05, "significant", "not significant")) %>% #Pvalue <0.05 indicate ada perbedaan antara data dalam kelompok (signifikan)
...
setDT(ourfish.l_malabaricus.cpue.1.glm2)

# Label Landings trends
ourfish.l_malabaricus.cpue.1.glm3 <- ourfish.l_malabaricus.cpue.1.glm2[,
                                                   trend := case_when(estimate >= 0 & stat_test == "significant" ~ "Increased", #estimate is to measure the alternative hypothesis, >=0 means there is positive linear relationship between predictor (date) and response (CPUE or weight)
                                                                      stat_test == "not significant" ~ "Stable",
                                                                      TRUE ~ "Decreased")]

trend.label <- ourfish.l_malabaricus.cpue.1.glm3[, `:=`(term = NULL, estimate = NULL, std.error = NULL, statistic = NULL, p.value = NULL, p_value_adjusted = NULL, stat_test = NULL)]

summary(ourfish.l_malabaricus.cpue.1.glm3)

ourfish.l_malabaricus.cpue.1.plot <- ggplot(ourfish.l_malabaricus.cpue.1, aes(x = date, y = sum_landings)) +
  geom_smooth(method = "loess", span = 0.5, color = "#e78828", fill = "#e78828") +
  scale_y_continuous(expand = c(0,0), limits = c(0,NA), oob = squish) +
  scale_x_date(labels = date_format("%b %Y"),
               date_breaks = "6 month") +
  geom_text(data = trend.label,
            ...
            #vjust = 5,
            inherit.aes = FALSE) +
  labs(title = "Catch Per Unit Effort L.malabaricus in MAR Mawasangka",
       x = "",
       y = "CPUE (kg/fisher/trip)") +
  theme.default.1 +
  theme(
...
cpue.Mawasangka.indo <- ourfish.l_malabaricus.cpue.1.plot + theme(
  plot.background = element_rect(fill = "transparent", colour = NA), # Makes the entire plot background transparent
  panel.background = element_rect(fill = "transparent", colour = NA) # Makes the panel background transparent
)
ggsave("cpue l_malabaricus Mawasangka.svg", plot = cpue.Mawasangka.indo, bg = "transparent", width = 13.33, height = 7.3)
