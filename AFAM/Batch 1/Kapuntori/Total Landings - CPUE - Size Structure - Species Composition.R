...
setwd("C:/Users/IrlanAssidiq/OneDrive - Rare/ourfish/AFAM/Batch 1/Kapuntori")
getwd()
...
#===============================================================================
# Total Landings
ourfish.Buton<- ourfish.raw[#month >= 2 & month <= 7 &
  lgu_name %in% "Buton"]

ourfish.ses.landings.Kapuntori <- ourfish.Kapuntori[ma_name %in% "Kapuntori",
                                                  .(Berat = sum(weight_kg)),
                                                  by = .(date, buying_unit, label)][order(date)]

setnames(ourfish.Kapuntori, "ma_name", "location")
ourfish.ses.landings.1 <- ourfish.Kapuntori[location %in% "Kapuntori",
...
ourfish.ses.landings.1.plot <- ggplot(ourfish.ses.landings.1, aes(x = date, y = sum_landings)) +
  ...
  geom_text(data = trend.label,
            aes(x = as.Date("2022-06-01", format("%Y-%m-%d")),
                y = 30,
                label = trend),
            hjust = 0.1,
...
landings.Kapuntori <- ourfish.ses.landings.1.plot + theme(
  plot.background = element_rect(fill = "transparent", colour = NA), # Makes the entire plot background transparent
  panel.background = element_rect(fill = "transparent", colour = NA) # Makes the panel background transparent
)
...
ourfish.ses.cpue.1 <- ourfish.Kapuntori[location %in% "Kapuntori",
                                       .(sum_landings = sum(weight_kg)),
...
cpue.Kapuntori <- ourfish.ses.cpue.1.plot + theme(
  plot.background = element_rect(fill = "transparent", colour = NA), # Makes the entire plot background transparent
  panel.background = element_rect(fill = "transparent", colour = NA) # Makes the panel background transparent
)
...
library(EnvStats)
quantile(ourfish.Buton$weight_kg, 0.01)
quantile(ourfish.Buton$weight_kg, 0.99)

# Number of k value for Rosner Test
num.k.upper.weight <- ourfish.Buton[weight_kg >= quantile(weight_kg, 0.99), .N]
num.k.lower.weight <- ourfish.Buton[weight_kg <= quantile(weight_kg, 0.01), .N]
k.value.rosnertest.weight <- num.k.upper.weight + num.k.lower.weight

# Statistical test for outliers using Rosner Test
rosner.test.landings.data <- rosnerTest(ourfish.Buton$weight_kg, k = k.value.rosnertest.weight)
...
ourfish.Buton.outlier <- ourfish.Buton[landings.data.outliers$Obs.Num, ]
...
ourfish.Buton.clean <- ourfish.Buton[-landings.data.outliers$Obs.Num, ]

## Box Plot for annual comparison

# Make data for two years period
ourfish.Buton.clean.boxplot.1 <- ourfish.Buton.clean[,
                                                     year_period := as.factor("2 Years Period")]

# Make data for Year 1
ourfish.Buton.clean.boxplot.2 <- ourfish.Buton.clean[date >= start.date & date <= "2022-10-01"][,
                                                                                                year_period := as.factor("Year 1")]

# Make data for Year 2
ourfish.Buton.clean.boxplot.3 <- ourfish.Buton.clean[date >= "2022-08-01" & date <= end.date][,
                                                                                              year_period := as.factor("Year 2")]

# Combine data of two years period with year 1 & 2 data
ourfish.Buton.clean.boxplot.4 <- rbind(ourfish.Buton.clean.boxplot.1, ourfish.Buton.clean.boxplot.2, ourfish.Buton.clean.boxplot.3)

# Annual CPUE & Catch Value using box plot
ourfish.Buton.clean.boxplot.5 <- ourfish.Buton.clean.boxplot.4[,
...
t.test(data = ourfish.Buton.clean.boxplot.5[!year_period == "2 Years Period"],
...
boxplot.annual.cpue <- 
  ggplot(data = ourfish.Buton.clean.boxplot.5,
...
ourfish.Buton.clean.boxplot.6 <- ourfish.Buton.clean.boxplot.5[,
                                                               .(n_trip = .N,
...
### Average length
ourfish.Buton.clean$ind_weight_g <- (ourfish.Buton.clean$weight_kg *1000)/ourfish.Buton.clean$count
ourfish.Buton.clean$length <- (ourfish.Buton.clean$ind_weight_g/ourfish.Buton.clean$a)^(1/ourfish.Buton.clean$b)

### average length by family
ourfish.Buton.prov.ma$ind_weight_g <- (ourfish.Buton.prov.ma$weight_kg *1000)/ourfish.Buton.prov.ma$count
ourfish.Buton.prov.ma$length <- (ourfish.Buton.prov.ma$ind_weight_g/ourfish.Buton.prov.ma$a)^(1/ourfish.Buton.prov.ma$b)


# Subset weight data by date, location, fisher_id, family
ourfish.Buton.length.fam.1 <- ourfish.Buton.prov.ma[,
                                                    .(mean_length_fam = mean(length)),
                                                    by = .(date, location, family)][order(date)]

ourfish.Buton.length.sp.1 <- ourfish.Buton.prov.ma[,
                                                   .(mean_length_sp = mean(length)),
                                                   by = .(date, location,family, species)][order(date)]

ourfish.Buton.length.sp.1 <- filter_if(ourfish.Buton.length.sp.1, is.numeric, all_vars((.) != 0))
table.sp.length <- table(c(ourfish.Buton.length.sp.1$species,
                           ourfish.Buton.length.sp.1$mean_length_sp))

# Rename level1_name and ma_name into the same column name to be combined
ourfish.Buton.combine.1 <- ourfish.Buton.clean[ , ]
ourfish.Buton.combine.2 <- ourfish.Buton.clean[ , ]

# Combine data from whole Buton with data by MA+R
ourfish.Buton.prov.ma <- rbind(ourfish.Buton.combine.1, ourfish.Buton.combine.2)
...
table.fam <- table(ourfish.Buton.prov.ma$family)
write.xlsx(table.fam, "C:/Users/IrlanAssidiq/OneDrive - Rare/Buton/plot/from 2019/famili_occ.xlsx")

# Change data into wide format
ourfish.Buton.length.fam.1.wide <- dcast(ourfish.Buton.length.fam.1,
                                         formula = date + location ~ family,
                                         value.var = "mean_length_fam")

ourfish.Buton.length.sp.1.wide <- dcast(ourfish.Buton.length.sp.1,
                                        formula = date + location + family ~ species,
                                        value.var = "mean_length_sp")

# Replace NA value with 0
ourfish.Buton.length.fam.1.wide[is.na(ourfish.Buton.length.fam.1.wide)] <- 0

ourfish.Buton.length.sp.1.wide[is.na(ourfish.Buton.length.sp.1.wide)] <- 0

# Change data back into long format
ourfish.Buton.length.fam.1.long <- melt(ourfish.Buton.length.fam.1.wide,
                                        measure.vars = c(3:20), # Please check the number of column first
                                        variable.name = "family",
                                        value.name = "mean_length_fam")
ourfish.Buton.length.fam.1.long.clean <- filter_if(ourfish.Buton.length.fam.1.long, is.numeric, all_vars((.) !=0))
ourfish.Buton.length.fam.1.long.clean <- filter_if(ourfish.Buton.length.fam.1.long.clean, is.numeric, all_vars((.) !=Inf))
ourfish.Buton.length.sp.1.long <- melt(ourfish.Buton.length.sp.1.wide,
                                       measure.vars = c(4:45), # Please check the number of column first
                                       variable.name = "species",
                                       value.name = "mean_length_sp")

ourfish.Buton.length.sp.1.long.clean <- filter_if(ourfish.Buton.length.sp.1.long, is.numeric, all_vars((.) != 0))
ourfish.Buton.length.sp.1.long.clean <- filter_if(ourfish.Buton.length.sp.1.long.clean, is.numeric, all_vars((.) !=Inf)) 

table.sp.clean <- table(ourfish.Buton.length.sp.1.long.clean$species)
write.xlsx(table.sp.clean, "C:/Users/IrlanAssidiq/OneDrive - Rare/species_occ_clean.xlsx")

#Size Structure
#creating new column for Year_month
library(zoo)
ourfish.Buton.clean$year_month <- ourfish.Buton.clean$date
ourfish.Buton.clean$year_month <- as.yearmon(ourfish.Buton.clean$year_month, format = "%Y_%m")

#creating midlength, using round up function
ourfish.Buton.clean$round_length <- round(ourfish.Buton.clean$length)

#crate a weekly catch data
library(lubridate)
#create weekly data from date
ourfish.Buton.clean$date <- as.Date(ourfish.Buton.clean$date)

ourfish.Buton.clean$week <- floor(ourfish.Buton.clean$date)

Buton.weekly <- ourfish.Buton.clean %>%
  mutate(week_start = floor_date(date, "week", week_start = 1))%>%
           group_by(week_start)%>%
           summarise("PAAP" = paste(unique(ma_name), collapse = ", "),
                     "Jumlah PAAP" = length(unique(ma_id)), 
                     "Desa" = paste(unique(community_name), collapse = ", "),
                     "Jumlah Desa" = length(unique(community_id)),
                     "Pembeli Ikan" = paste(unique(buyer_name), collapse = ", "),
                     "Jumlah Pembeli Ikan" = length(unique(buyer_id)),
                     "Total Tangkapan Mingguan" = sum(weight_kg),
                     "Rata-rata Tangkapan Mingguan" = mean(weight_kg),
                     )%>%
           ungroup()

Buton.weekly.1 <- ourfish.Buton.clean[,
                                      .(weight = sum(weight_kg)),
                                      by = .(date, year, month, ma_name, community_name, buyer_name)]

Buton.weekly.2 <- Buton.weekly.1[,
                                 .(date = floor_date(date, "week", week_start = 1)),
                                 by = .(ma_name, year, month, community_name, buyer_name, weight)]
Buton.weekly.3 <- Buton.weekly.2[,
                                 .(sum_weight = sum(weight)),
                                 by = .(date, year, month, ma_name, community_name, buyer_name)] [order(date)]

write.xlsx(Buton.weekly.3,"C:/Users/IrlanAssidiq/OneDrive - Rare/buton fish production weekly basis.xlsx")

Buton.monthly.1 <- ourfish.Buton.clean[,
                                       .(weight = sum(weight_kg)),
                                       by = . (year_month, ma_name, community_name, buyer_name)][order(year_month)]

write.xlsx(Buton.monthly.1,"C:/Users/IrlanAssidiq/OneDrive - Rare/buton fish production monthly.xlsx")


buton.weekly.plot <- ggplot(Buton.weekly, aes(x=week_start, y = weekly_avg)) +
  ...
  labs(title = "Buton weekly Fish Production average (in Kg)",
       subtitle = "recorded in Ourfish App",
       x = " ",
       y = "fish production average (kg)") +
  theme.default.1 +
  theme(
    plot.background = element_rect(fill = "transparent", colour = NA), # Makes the entire plot background transparent
    panel.background = element_rect(fill = "transparent", colour = NA) # Makes the panel background transparent
...
buton.weekly.plot
ggsave("Buton Production.svg", plot = buton.weekly.plot, bg = "transparent", width = 13.33, height = 7.3)

write.xlsx(Buton.weekly,"C:/Users/IrlanAssidiq/OneDrive - Rare/Buton weekly production.xlsx" )

#Katamba - Lethrinus lentjan
ourfish.Buton.l_lentjan <- ourfish.Buton.clean[species%in% "Lethrinus lentjan",
                                                         .(count = sum(count)),
                                                         by = .(year_month, round_length)] [order(year_month)]
ourfish.Buton.l_lentjan$year_month <- as.Date(ourfish.Buton.l_lentjan$year_month)
ourfish.Buton.l_lentjan.wide <- ourfish.Buton.l_lentjan %>%
  pivot_wider(names_from = year_month,
              values_from = count,
              values_fill = 0)

ourfish.Buton.l_lentjan.sorted <- ourfish.Buton.l_lentjan.wide %>% arrange(round_length)

#===============================================================================
#Data arrangement: agar bisa dibaca sama R
dates <- colnames(ourfish.Buton.l_lentjan.sorted)[-1]

#Mari definisikan format si tanggal, sesuaikan dengan format data mentah
dates <- as.Date(dates, "%Y-%m-%d")
#New data frame from length.series data frame to be read by TropFishR
length.series.data <- list(dates = dates,
                           midLengths = ourfish.Buton.l_lentjan.sorted$round_length,
                           catch = as.matrix(ourfish.Buton.l_lentjan.sorted[,-1]))
...
max.length = as.numeric(max(ourfish.Buton.l_lentjan.sorted$round_length))
...
#Nilai K bisa pake fishlife juga untuk Kmin dan Kmax, biasanya range 0-1, makin slow-growth makin rendah angkanya, cek masuk akal ga sesuai biologi spesies
K.fishlife = 0.3787560 #Input K value from Fish Life script or other literatures/sources
...
length.series.data$M = 0.527

#preparing raw data for length series
ourfish.Buton.l_lentjan.long <- ourfish.Buton.l_lentjan.sorted %>%
  pivot_longer(
    cols = starts_with("20"),
    names_to = "dates",
    values_to = "value")

ourfish.Buton.l_lentjan.long.2 <- ourfish.Buton.l_lentjan.long %>%
  uncount(weights = value)

#Raw Data
length.raw <- tibble(Length = ourfish.Buton.l_lentjan.long.2$round_length)
...
hasil$linear_mod_sel = NULL; hasil$Sobs = NULL; hasil$ln_1_S_1 = NULL; hasil$Sest = NULL
...
ggsave("l_lentjan size.svg", plot = size.structure.plot, bg = "transparent", width = 13.33, height = 7.3)

#total landings & CPUE L-lentjan================================================
ourfish.l_lentjan.landings.1 <- ourfish.Buton.clean[species %in% "Lethrinus lentjan",
                                            .(sum_landings = sum(weight_kg)),
                                            by = .(date, species)][order(date)]

ourfish.l_lentjan.landings.1.glm <- ourfish.l_lentjan.landings.1 %>%
  nest(-'species') %>%
  mutate(model = map(data, ~ glm(sum_landings ~ date, data = .))) %>%
  mutate(tidied = map(model, tidy)) %>%
  unnest(tidied)

ourfish.l_lentjan.landings.1.glm2 <- ourfish.l_lentjan.landings.1.glm %>%
  dplyr::filter(term == "date") %>%
  mutate(p_value_adjusted = p.adjust(p.value)) %>%
  arrange(p_value_adjusted) %>%
  mutate(stat_test = ifelse(p_value_adjusted < 0.05, "significant", "not significant")) %>% #Pvalue <0.05 indicate ada perbedaan antara data dalam kelompok (signifikan)
...
setDT(ourfish.l_lentjan.landings.1.glm2)

# Label Landings trends
ourfish.l_lentjan.landings.1.glm3 <- ourfish.l_lentjan.landings.1.glm2[,
                                                           trend := case_when(estimate >= 0 & stat_test == "significant" ~ "Increased", #estimate is to measure the alternative hypothesis, >=0 means there is positive linear relationship between predictor (date) and response (CPUE or weight)
                                                                              stat_test == "not significant" ~ "Stable",
                                                                              TRUE ~ "Decreased")]

trend.label <- ourfish.l_lentjan.landings.1.glm3[, `:=`(term = NULL, estimate = NULL, std.error = NULL, statistic = NULL, p.value = NULL, p_value_adjusted = NULL, stat_test = NULL)]

summary(ourfish.l_lentjan.landings.1.glm3)

ourfish.l_lentjan.landings.1.plot <- ggplot(ourfish.l_lentjan.landings.1, aes(x = date, y = sum_landings)) +
  ...
  geom_text(data = trend.label,
            ...
            #vjust = 5,
            inherit.aes = FALSE) +
  labs(title = "Total Landings L.lentjan in MAR Buton",
       x = "",
       y = "Total landings(kg)") +
  theme.default.1 +
  theme(
...
landings.Buton.indo <- ourfish.l_lentjan.landings.1.plot + theme(
  plot.background = element_rect(fill = "transparent", colour = NA), # Makes the entire plot background transparent
  panel.background = element_rect(fill = "transparent", colour = NA) # Makes the panel background transparent
)
ggsave("landings l_lentjan Buton.svg", plot = landings.Buton.indo, bg = "transparent", width = 13.33, height = 7.3)


#### CPUE Trends in Southeast Sulawesi ####
# Subset weight data by date, species for total landings
ourfish.l_lentjan.cpue.1 <- ourfish.Buton.clean[species %in% "Lethrinus lentjan",
                                                    .(sum_landings = sum(weight_kg)),
                                                    by = .(date, species, fisher_id)][order(date)]

ourfish.l_lentjan.cpue.1.glm <- ourfish.l_lentjan.cpue.1 %>%
  nest(-species) %>%
  mutate(model = map(data, ~ glm(sum_landings ~ date, data = .))) %>%
  mutate(tidied = map(model, tidy)) %>%
  unnest(tidied)

ourfish.l_lentjan.cpue.1.glm2 <- ourfish.l_lentjan.cpue.1.glm %>%
  dplyr::filter(term == "date") %>%
  mutate(p_value_adjusted = p.adjust(p.value)) %>%
  arrange(p_value_adjusted) %>%
  mutate(stat_test = ifelse(p_value_adjusted < 0.05, "significant", "not significant")) %>% #Pvalue <0.05 indicate ada perbedaan antara data dalam kelompok (signifikan)
...
setDT(ourfish.l_lentjan.cpue.1.glm2)

# Label Landings trends
ourfish.l_lentjan.cpue.1.glm3 <- ourfish.l_lentjan.cpue.1.glm2[,
                                                   trend := case_when(estimate >= 0 & stat_test == "significant" ~ "Increased", #estimate is to measure the alternative hypothesis, >=0 means there is positive linear relationship between predictor (date) and response (CPUE or weight)
                                                                      stat_test == "not significant" ~ "Stable",
                                                                      TRUE ~ "Decreased")]

trend.label <- ourfish.l_lentjan.cpue.1.glm3[, `:=`(term = NULL, estimate = NULL, std.error = NULL, statistic = NULL, p.value = NULL, p_value_adjusted = NULL, stat_test = NULL)]

summary(ourfish.l_lentjan.cpue.1.glm3)

ourfish.l_lentjan.cpue.1.plot <- ggplot(ourfish.l_lentjan.cpue.1, aes(x = date, y = sum_landings)) +
  geom_smooth(method = "loess", span = 0.5, color = "#e78828", fill = "#e78828") +
  scale_y_continuous(expand = c(0,0), limits = c(0,NA), oob = squish) +
  scale_x_date(labels = date_format("%b %Y"),
               date_breaks = "6 month") +
  geom_text(data = trend.label,
            ...
            #vjust = 5,
            inherit.aes = FALSE) +
  labs(title = "Catch Per Unit Effort L.lentjan in MAR Buton",
       x = "",
       y = "CPUE (kg/fisher/trip)") +
  theme.default.1 +
  theme(
...
cpue.Buton.indo <- ourfish.l_lentjan.cpue.1.plot + theme(
  plot.background = element_rect(fill = "transparent", colour = NA), # Makes the entire plot background transparent
  panel.background = element_rect(fill = "transparent", colour = NA) # Makes the panel background transparent
)
ggsave("cpue l_lentjan Buton.svg", plot = cpue.Buton.indo, bg = "transparent", width = 13.33, height = 7.3)
