...
#Katamba - Lutjanus vitta
ourfish.Kapuntori.L_vitta <- ourfish.Kapuntori.clean[species%in% "Lutjanus vitta",
                                                         .(count = sum(count)),
                                                         by = .(year_month, round_length)] [order(year_month)]
ourfish.Kapuntori.L_vitta$year_month <- as.Date(ourfish.Kapuntori.L_vitta$year_month)
ourfish.Kapuntori.L_vitta.wide <- ourfish.Kapuntori.L_vitta %>%
  pivot_wider(names_from = year_month,
              values_from = count,
              values_fill = 0)

ourfish.Kapuntori.L_vitta.sorted <- ourfish.Kapuntori.L_vitta.wide %>% arrange(round_length)

#===============================================================================
#Data arrangement: agar bisa dibaca sama R
dates <- colnames(ourfish.Kapuntori.L_vitta.sorted)[-1]
...
length.series.data <- list(dates = dates,
                           midLengths = ourfish.Kapuntori.L_vitta.sorted$round_length,
                           catch = as.matrix(ourfish.Kapuntori.L_vitta.sorted[,-1]))
...
max.length = as.numeric(max(ourfish.Kapuntori.L_vitta.sorted$round_length))
...
K.fishlife = 0.3599932 #Input K value from Fish Life script or other literatures/sources
...
length.series.data$M = 0.53

#preparing raw data for length series
ourfish.Kapuntori.L_vitta.long <- ourfish.Kapuntori.L_vitta.sorted %>%
  pivot_longer(
    cols = starts_with("20"),
    names_to = "dates",
    values_to = "value")

ourfish.Kapuntori.L_vitta.long.2 <- ourfish.Kapuntori.L_vitta.long %>%
  uncount(weights = value)

#Raw Data
length.raw <- tibble(Length = ourfish.Kapuntori.L_vitta.long.2$round_length)
...
ggsave("L_vitta size.svg", plot = size.structure.plot, bg = "transparent", width = 13.33, height = 7.3)

#total landings & CPUE L-lentjan================================================
ourfish.L_vitta.landings.1 <- ourfish.Kapuntori.clean[species %in% "Lutjanus vitta",
                                            .(sum_landings = sum(weight_kg)),
                                            by = .(date, species)][order(date)]

ourfish.L_vitta.landings.1.glm <- ourfish.L_vitta.landings.1 %>%
  nest(-'species') %>%
  mutate(model = map(data, ~ glm(sum_landings ~ date, data = .))) %>%
  mutate(tidied = map(model, tidy)) %>%
  unnest(tidied)

ourfish.L_vitta.landings.1.glm2 <- ourfish.L_vitta.landings.1.glm %>%
  dplyr::filter(term == "date") %>%
  mutate(p_value_adjusted = p.adjust(p.value)) %>%
  arrange(p_value_adjusted) %>%
  mutate(stat_test = ifelse(p_value_adjusted < 0.05, "significant", "not significant")) %>% #Pvalue <0.05 indicate ada perbedaan antara data dalam kelompok (signifikan)
...
setDT(ourfish.L_vitta.landings.1.glm2)

# Label Landings trends
ourfish.L_vitta.landings.1.glm3 <- ourfish.L_vitta.landings.1.glm2[,
                                                           trend := case_when(estimate >= 0 & stat_test == "significant" ~ "Increased", #estimate is to measure the alternative hypothesis, >=0 means there is positive linear relationship between predictor (date) and response (CPUE or weight)
                                                                              stat_test == "not significant" ~ "Stable",
                                                                              TRUE ~ "Decreased")]

trend.label <- ourfish.L_vitta.landings.1.glm3[, `:=`(term = NULL, estimate = NULL, std.error = NULL, statistic = NULL, p.value = NULL, p_value_adjusted = NULL, stat_test = NULL)]

summary(ourfish.L_vitta.landings.1.glm3)

ourfish.L_vitta.landings.1.plot <- ggplot(ourfish.L_vitta.landings.1, aes(x = date, y = sum_landings)) +
  ...
  geom_text(data = trend.label,
            ...
            #vjust = 5,
            inherit.aes = FALSE) +
  labs(title = "Total Landings L.vitta in MAR Kapuntori",
       x = "",
       y = "Total landings(kg)") +
  theme.default.1 +
  theme(
...
landings.Kapuntori.indo <- ourfish.L_vitta.landings.1.plot + theme(
  plot.background = element_rect(fill = "transparent", colour = NA), # Makes the entire plot background transparent
  panel.background = element_rect(fill = "transparent", colour = NA) # Makes the panel background transparent
)
ggsave("landings L_vitta Kapuntori.svg", plot = landings.Kapuntori.indo, bg = "transparent", width = 13.33, height = 7.3)


#### CPUE Trends in Southeast Sulawesi ####
# Subset weight data by date, species for total landings
ourfish.L_vitta.cpue.1 <- ourfish.Kapuntori.clean[species %in% "Lutjanus vitta",
                                                    .(sum_landings = sum(weight_kg)),
                                                    by = .(date, species, fisher_id)][order(date)]

ourfish.L_vitta.cpue.1.glm <- ourfish.L_vitta.cpue.1 %>%
  nest(-species) %>%
  mutate(model = map(data, ~ glm(sum_landings ~ date, data = .))) %>%
  mutate(tidied = map(model, tidy)) %>%
  unnest(tidied)

ourfish.L_vitta.cpue.1.glm2 <- ourfish.L_vitta.cpue.1.glm %>%
  dplyr::filter(term == "date") %>%
  mutate(p_value_adjusted = p.adjust(p.value)) %>%
  arrange(p_value_adjusted) %>%
  mutate(stat_test = ifelse(p_value_adjusted < 0.05, "significant", "not significant")) %>% #Pvalue <0.05 indicate ada perbedaan antara data dalam kelompok (signifikan)
...
setDT(ourfish.L_vitta.cpue.1.glm2)

# Label Landings trends
ourfish.L_vitta.cpue.1.glm3 <- ourfish.L_vitta.cpue.1.glm2[,
                                                   trend := case_when(estimate >= 0 & stat_test == "significant" ~ "Increased", #estimate is to measure the alternative hypothesis, >=0 means there is positive linear relationship between predictor (date) and response (CPUE or weight)
                                                                      stat_test == "not significant" ~ "Stable",
                                                                      TRUE ~ "Decreased")]

trend.label <- ourfish.L_vitta.cpue.1.glm3[, `:=`(term = NULL, estimate = NULL, std.error = NULL, statistic = NULL, p.value = NULL, p_value_adjusted = NULL, stat_test = NULL)]

summary(ourfish.L_vitta.cpue.1.glm3)

ourfish.L_vitta.cpue.1.plot <- ggplot(ourfish.L_vitta.cpue.1, aes(x = date, y = sum_landings)) +
  geom_smooth(method = "loess", span = 0.5, color = "#B50A37", fill = "#B50A37") +
  scale_y_continuous(expand = c(0,0), limits = c(0,NA), oob = squish) +
  scale_x_date(labels = date_format("%b %Y"),
               date_breaks = "6 month") +
  geom_text(data = trend.label,
            ...
            #vjust = 5,
            inherit.aes = FALSE) +
  labs(title = "Catch Per Unit Effort L.vitta in MAR Kapuntori",
       x = "",
       y = "CPUE (kg/fisher/trip)") +
  theme.default.1 +
  theme(
...
cpue.Kapuntori.indo <- ourfish.L_vitta.cpue.1.plot + theme(
  plot.background = element_rect(fill = "transparent", colour = NA), # Makes the entire plot background transparent
  panel.background = element_rect(fill = "transparent", colour = NA) # Makes the panel background transparent
)
ggsave("cpue L_vitta Kapuntori.svg", plot = cpue.Kapuntori.indo, bg = "transparent", width = 13.33, height = 7.3)
