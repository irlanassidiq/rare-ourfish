...
setwd("C:/Users/IrlanAssidiq/OneDrive - Rare/ourfish/AFAM/Kapuntori")
getwd()
...
#===============================================================================
# Total Landings
ourfish.Kapuntori<- ourfish.raw[#month >= 2 & month <= 7 &
...
library(EnvStats)
quantile(ourfish.Kapuntori$weight_kg, 0.01)
quantile(ourfish.Kapuntori$weight_kg, 0.99)

# Number of k value for Rosner Test
num.k.upper.weight <- ourfish.Kapuntori[weight_kg >= quantile(weight_kg, 0.99), .N]
num.k.lower.weight <- ourfish.Kapuntori[weight_kg <= quantile(weight_kg, 0.01), .N]
k.value.rosnertest.weight <- num.k.upper.weight + num.k.lower.weight

# Statistical test for outliers using Rosner Test
rosner.test.landings.data <- rosnerTest(ourfish.Kapuntori$weight_kg, k = k.value.rosnertest.weight)
...
ourfish.Kapuntori.outlier <- ourfish.Kapuntori[landings.data.outliers$Obs.Num, ]
...
ourfish.Kapuntori.clean <- ourfish.Kapuntori[-landings.data.outliers$Obs.Num, ]

## Box Plot for annual comparison

# Make data for two years period
ourfish.Kapuntori.clean.boxplot.1 <- ourfish.Kapuntori.clean[,
                                                     year_period := as.factor("2 Years Period")]

# Make data for Year 1
ourfish.Kapuntori.clean.boxplot.2 <- ourfish.Kapuntori.clean[date >= start.date & date <= "2022-10-01"][,
                                                                                                year_period := as.factor("Year 1")]

# Make data for Year 2
ourfish.Kapuntori.clean.boxplot.3 <- ourfish.Kapuntori.clean[date >= "2022-08-01" & date <= end.date][,
                                                                                              year_period := as.factor("Year 2")]

# Combine data of two years period with year 1 & 2 data
ourfish.Kapuntori.clean.boxplot.4 <- rbind(ourfish.Kapuntori.clean.boxplot.1, ourfish.Kapuntori.clean.boxplot.2, ourfish.Kapuntori.clean.boxplot.3)

# Annual CPUE & Catch Value using box plot
ourfish.Kapuntori.clean.boxplot.5 <- ourfish.Kapuntori.clean.boxplot.4[,
...
ourfish.Kapuntori.clean.boxplot.5[,
...
t.test(data = ourfish.Kapuntori.clean.boxplot.5[!year_period == "2 Years Period"],
...
boxplot.annual.cpue <- 
  ggplot(data = ourfish.Kapuntori.clean.boxplot.5,
...
ourfish.Kapuntori.clean.boxplot.6 <- ourfish.Kapuntori.clean.boxplot.5[,
                                                               .(n_trip = .N,
...
fwrite(ourfish.Kapuntori.clean.boxplot.6,
...
### Average length
ourfish.Kapuntori.clean$ind_weight_g <- (ourfish.Kapuntori.clean$weight_kg *1000)/ourfish.Kapuntori.clean$count
ourfish.Kapuntori.clean$length <- (ourfish.Kapuntori.clean$ind_weight_g/ourfish.Kapuntori.clean$a)^(1/ourfish.Kapuntori.clean$b)

### average length by family
ourfish.Kapuntori.prov.ma$ind_weight_g <- (ourfish.Kapuntori.prov.ma$weight_kg *1000)/ourfish.Kapuntori.prov.ma$count
ourfish.Kapuntori.prov.ma$length <- (ourfish.Kapuntori.prov.ma$ind_weight_g/ourfish.Kapuntori.prov.ma$a)^(1/ourfish.Kapuntori.prov.ma$b)


# Subset weight data by date, location, fisher_id, family
ourfish.Kapuntori.length.fam.1 <- ourfish.Kapuntori.prov.ma[,
                                                    .(mean_length_fam = mean(length)),
                                                    by = .(date, location, family)][order(date)]

ourfish.Kapuntori.length.sp.1 <- ourfish.Kapuntori.prov.ma[,
                                                   .(mean_length_sp = mean(length)),
                                                   by = .(date, location,family, species)][order(date)]

ourfish.Kapuntori.length.sp.1 <- filter_if(ourfish.Kapuntori.length.sp.1, is.numeric, all_vars((.) != 0))
table.sp.length <- table(c(ourfish.Kapuntori.length.sp.1$species,
                           ourfish.Kapuntori.length.sp.1$mean_length_sp))

# Rename level1_name and ma_name into the same column name to be combined
ourfish.Kapuntori.combine.1 <- ourfish.Kapuntori.clean[ , ]
ourfish.Kapuntori.combine.2 <- ourfish.Kapuntori.clean[ , ]

# Combine data from whole Kapuntori with data by MA+R
ourfish.Kapuntori.prov.ma <- rbind(ourfish.Kapuntori.combine.1, ourfish.Kapuntori.combine.2)
...
table.fam <- table(ourfish.Kapuntori.prov.ma$family)
write.xlsx(table.fam, "C:/Users/IrlanAssidiq/OneDrive - Rare/Kapuntori/plot/from 2019/famili_occ.xlsx")

# Change data into wide format
ourfish.Kapuntori.length.fam.1.wide <- dcast(ourfish.Kapuntori.length.fam.1,
                                         formula = date + location ~ family,
                                         value.var = "mean_length_fam")

ourfish.Kapuntori.length.sp.1.wide <- dcast(ourfish.Kapuntori.length.sp.1,
                                        formula = date + location + family ~ species,
                                        value.var = "mean_length_sp")

# Replace NA value with 0
ourfish.Kapuntori.length.fam.1.wide[is.na(ourfish.Kapuntori.length.fam.1.wide)] <- 0

ourfish.Kapuntori.length.sp.1.wide[is.na(ourfish.Kapuntori.length.sp.1.wide)] <- 0

# Change data back into long format
ourfish.Kapuntori.length.fam.1.long <- melt(ourfish.Kapuntori.length.fam.1.wide,
                                        measure.vars = c(3:20), # Please check the number of column first
                                        variable.name = "family",
                                        value.name = "mean_length_fam")
ourfish.Kapuntori.length.fam.1.long.clean <- filter_if(ourfish.Kapuntori.length.fam.1.long, is.numeric, all_vars((.) !=0))
ourfish.Kapuntori.length.fam.1.long.clean <- filter_if(ourfish.Kapuntori.length.fam.1.long.clean, is.numeric, all_vars((.) !=Inf))
ourfish.Kapuntori.length.sp.1.long <- melt(ourfish.Kapuntori.length.sp.1.wide,
                                       measure.vars = c(4:45), # Please check the number of column first
                                       variable.name = "species",
                                       value.name = "mean_length_sp")

ourfish.Kapuntori.length.sp.1.long.clean <- filter_if(ourfish.Kapuntori.length.sp.1.long, is.numeric, all_vars((.) != 0))
ourfish.Kapuntori.length.sp.1.long.clean <- filter_if(ourfish.Kapuntori.length.sp.1.long.clean, is.numeric, all_vars((.) !=Inf)) 

table.sp.clean <- table(ourfish.Kapuntori.length.sp.1.long.clean$species)
write.xlsx(table.sp.clean, "C:/Users/IrlanAssidiq/OneDrive - Rare/species_occ_clean.xlsx")

#Size Structure
#creating new column for Year_month
library(zoo)
ourfish.Kapuntori.clean$year_month <- ourfish.Kapuntori.clean$date
ourfish.Kapuntori.clean$year_month <- as.yearmon(ourfish.Kapuntori.clean$year_month, format = "%Y_%m")

#creating midlength, using round up function
ourfish.Kapuntori.clean$round_length <- round(ourfish.Kapuntori.clean$length)

#Katamba - Priacanthus macracanthus
ourfish.Kapuntori.p_macracanthus <- ourfish.Kapuntori.clean[species%in% "Priacanthus macracanthus",
                                                         .(count = sum(count)),
                                                         by = .(year_month, round_length)] [order(year_month)]
ourfish.Kapuntori.p_macracanthus$year_month <- as.Date(ourfish.Kapuntori.p_macracanthus$year_month)
ourfish.Kapuntori.p_macracanthus.wide <- ourfish.Kapuntori.p_macracanthus %>%
  pivot_wider(names_from = year_month,
              values_from = count,
              values_fill = 0)

ourfish.Kapuntori.p_macracanthus.sorted <- ourfish.Kapuntori.p_macracanthus.wide %>% arrange(round_length)

#===============================================================================
#Data arrangement: agar bisa dibaca sama R
dates <- colnames(ourfish.Kapuntori.p_macracanthus.sorted)[-1]
...
length.series.data <- list(dates = dates,
                           midLengths = ourfish.Kapuntori.p_macracanthus.sorted$round_length,
                           catch = as.matrix(ourfish.Kapuntori.p_macracanthus.sorted[,-1]))
...
max.length = as.numeric(max(ourfish.Kapuntori.p_macracanthus.sorted$round_length))
...
K.fishlife = 0.485553 #Input K value from Fish Life script or other literatures/sources
...
length.series.data$M = 0.6

#preparing raw data for length series
ourfish.Kapuntori.p_macracanthus.long <- ourfish.Kapuntori.p_macracanthus.sorted %>%
  pivot_longer(
    cols = starts_with("20"),
    names_to = "dates",
    values_to = "value")

ourfish.Kapuntori.p_macracanthus.long.2 <- ourfish.Kapuntori.p_macracanthus.long %>%
  uncount(weights = value)

#Raw Data
length.raw <- tibble(Length = ourfish.Kapuntori.p_macracanthus.long.2$round_length)
...
ggsave("p_macracanthus size.svg", plot = size.structure.plot, bg = "transparent", width = 13.33, height = 7.3)

#total landings & CPUE L-lentjan================================================
ourfish.p_macracanthus.landings.1 <- ourfish.Kapuntori.clean[species %in% "Priacanthus macracanthus",
                                            .(sum_landings = sum(weight_kg)),
                                            by = .(date, species)][order(date)]

ourfish.p_macracanthus.landings.1.glm <- ourfish.p_macracanthus.landings.1 %>%
  nest(-'species') %>%
  mutate(model = map(data, ~ glm(sum_landings ~ date, data = .))) %>%
  mutate(tidied = map(model, tidy)) %>%
  unnest(tidied)

ourfish.p_macracanthus.landings.1.glm2 <- ourfish.p_macracanthus.landings.1.glm %>%
  dplyr::filter(term == "date") %>%
  mutate(p_value_adjusted = p.adjust(p.value)) %>%
  arrange(p_value_adjusted) %>%
  mutate(stat_test = ifelse(p_value_adjusted < 0.05, "significant", "not significant")) %>% #Pvalue <0.05 indicate ada perbedaan antara data dalam kelompok (signifikan)
...
setDT(ourfish.p_macracanthus.landings.1.glm2)

# Label Landings trends
ourfish.p_macracanthus.landings.1.glm3 <- ourfish.p_macracanthus.landings.1.glm2[,
                                                           trend := case_when(estimate >= 0 & stat_test == "significant" ~ "Increased", #estimate is to measure the alternative hypothesis, >=0 means there is positive linear relationship between predictor (date) and response (CPUE or weight)
                                                                              stat_test == "not significant" ~ "Stable",
                                                                              TRUE ~ "Decreased")]

trend.label <- ourfish.p_macracanthus.landings.1.glm3[, `:=`(term = NULL, estimate = NULL, std.error = NULL, statistic = NULL, p.value = NULL, p_value_adjusted = NULL, stat_test = NULL)]

summary(ourfish.p_macracanthus.landings.1.glm3)

ourfish.p_macracanthus.landings.1.plot <- ggplot(ourfish.p_macracanthus.landings.1, aes(x = date, y = sum_landings)) +
  ...
  geom_text(data = trend.label,
            aes(x = as.Date("2022-06-01", format("%Y-%m-%d")),
                ...
                label = trend),
            hjust = 0.1,
            color = "#B50A37", #Orange #e78828; Hijau #00833e; Tosca #06a7b4; Red #B50A37
            size = 6,
            #vjust = 5,
            inherit.aes = FALSE) +
  labs(title = "Total Landings P.macracanthus in MAR Kapuntori",
       x = "",
       y = "Total landings(kg)") +
  theme.default.1 +
  theme(
...
landings.Kapuntori.indo <- ourfish.p_macracanthus.landings.1.plot + theme(
  plot.background = element_rect(fill = "transparent", colour = NA), # Makes the entire plot background transparent
  panel.background = element_rect(fill = "transparent", colour = NA) # Makes the panel background transparent
)
ggsave("landings p_macracanthus Kapuntori.svg", plot = landings.Kapuntori.indo, bg = "transparent", width = 13.33, height = 7.3)


#### CPUE Trends in Southeast Sulawesi ####
# Subset weight data by date, species for total landings
ourfish.p_macracanthus.cpue.1 <- ourfish.Kapuntori.clean[species %in% "Priacanthus macracanthus",
                                                    .(sum_landings = sum(weight_kg)),
                                                    by = .(date, species, fisher_id)][order(date)]

ourfish.p_macracanthus.cpue.1.glm <- ourfish.p_macracanthus.cpue.1 %>%
  nest(-species) %>%
  mutate(model = map(data, ~ glm(sum_landings ~ date, data = .))) %>%
  mutate(tidied = map(model, tidy)) %>%
  unnest(tidied)

ourfish.p_macracanthus.cpue.1.glm2 <- ourfish.p_macracanthus.cpue.1.glm %>%
  dplyr::filter(term == "date") %>%
  mutate(p_value_adjusted = p.adjust(p.value)) %>%
  arrange(p_value_adjusted) %>%
  mutate(stat_test = ifelse(p_value_adjusted < 0.05, "significant", "not significant")) %>% #Pvalue <0.05 indicate ada perbedaan antara data dalam kelompok (signifikan)
...
setDT(ourfish.p_macracanthus.cpue.1.glm2)

# Label Landings trends
ourfish.p_macracanthus.cpue.1.glm3 <- ourfish.p_macracanthus.cpue.1.glm2[,
                                                   trend := case_when(estimate >= 0 & stat_test == "significant" ~ "Increased", #estimate is to measure the alternative hypothesis, >=0 means there is positive linear relationship between predictor (date) and response (CPUE or weight)
                                                                      stat_test == "not significant" ~ "Stable",
                                                                      TRUE ~ "Decreased")]

trend.label <- ourfish.p_macracanthus.cpue.1.glm3[, `:=`(term = NULL, estimate = NULL, std.error = NULL, statistic = NULL, p.value = NULL, p_value_adjusted = NULL, stat_test = NULL)]

summary(ourfish.p_macracanthus.cpue.1.glm3)

ourfish.p_macracanthus.cpue.1.plot <- ggplot(ourfish.p_macracanthus.cpue.1, aes(x = date, y = sum_landings)) +
  geom_smooth(method = "loess", span = 0.5, color = "#B50A37", fill = "#B50A37") +
  scale_y_continuous(expand = c(0,0), limits = c(0,NA), oob = squish) +
  scale_x_date(labels = date_format("%b %Y"),
               date_breaks = "6 month") +
  geom_text(data = trend.label,
            ...
            #vjust = 5,
            inherit.aes = FALSE) +
  labs(title = "Catch Per Unit Effort P.macracanthus in MAR Kapuntori",
       x = "",
       y = "CPUE (kg/fisher/trip)") +
  theme.default.1 +
  theme(
...
cpue.Kapuntori.indo <- ourfish.p_macracanthus.cpue.1.plot + theme(
  plot.background = element_rect(fill = "transparent", colour = NA), # Makes the entire plot background transparent
  panel.background = element_rect(fill = "transparent", colour = NA) # Makes the panel background transparent
)
ggsave("cpue p_macracanthus Kapuntori.svg", plot = cpue.Kapuntori.indo, bg = "transparent", width = 13.33, height = 7.3)
