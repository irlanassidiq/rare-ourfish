...
ourfish.Kapuntori.l_lentjan <- ourfish.Kapuntori.clean[species%in% "Lethrinus lentjan",
                                                         .(count = sum(count)),
                                                         by = .(year_month, round_length)] [order(year_month)]
ourfish.Kapuntori.l_lentjan$year_month <- as.Date(ourfish.Kapuntori.l_lentjan$year_month)
ourfish.Kapuntori.l_lentjan.wide <- ourfish.Kapuntori.l_lentjan %>%
  pivot_wider(names_from = year_month,
              values_from = count,
              values_fill = 0)

ourfish.Kapuntori.l_lentjan.sorted <- ourfish.Kapuntori.l_lentjan.wide %>% arrange(round_length)

#===============================================================================
#Data arrangement: agar bisa dibaca sama R
dates <- colnames(ourfish.Kapuntori.l_lentjan.sorted)[-1]
...
length.series.data <- list(dates = dates,
                           midLengths = ourfish.Kapuntori.l_lentjan.sorted$round_length,
                           catch = as.matrix(ourfish.Kapuntori.l_lentjan.sorted[,-1]))
...
max.length = as.numeric(max(ourfish.Kapuntori.l_lentjan.sorted$round_length))
...
ourfish.Kapuntori.l_lentjan.long <- ourfish.Kapuntori.l_lentjan.sorted %>%
  pivot_longer(
    cols = starts_with("20"),
    names_to = "dates",
    values_to = "value")

ourfish.Kapuntori.l_lentjan.long.2 <- ourfish.Kapuntori.l_lentjan.long %>%
  uncount(weights = value)

#Raw Data
length.raw <- tibble(Length = ourfish.Kapuntori.l_lentjan.long.2$round_length)
...
ourfish.l_lentjan.landings.1 <- ourfish.Kapuntori.clean[species %in% "Lethrinus lentjan",
                                            .(sum_landings = sum(weight_kg)),
...
ourfish.l_lentjan.landings.1.plot <- ggplot(ourfish.l_lentjan.landings.1, aes(x = date, y = sum_landings)) +
  ...
  geom_text(data = trend.label,
            ...
            #vjust = 5,
            inherit.aes = FALSE) +
  labs(title = "Total Landings L.lentjan in MAR Kapuntori",
       x = "",
       y = "Total landings(kg)") +
  theme.default.1 +
  theme(
...
landings.Kapuntori.indo <- ourfish.l_lentjan.landings.1.plot + theme(
  plot.background = element_rect(fill = "transparent", colour = NA), # Makes the entire plot background transparent
  panel.background = element_rect(fill = "transparent", colour = NA) # Makes the panel background transparent
)
ggsave("landings l_lentjan Kapuntori.svg", plot = landings.Kapuntori.indo, bg = "transparent", width = 13.33, height = 7.3)


#### CPUE Trends in Southeast Sulawesi ####
# Subset weight data by date, species for total landings
ourfish.l_lentjan.cpue.1 <- ourfish.Kapuntori.clean[species %in% "Lethrinus lentjan",
                                                    .(sum_landings = sum(weight_kg)),
...
ourfish.l_lentjan.cpue.1.plot <- ggplot(ourfish.l_lentjan.cpue.1, aes(x = date, y = sum_landings)) +
  ...
  geom_text(data = trend.label,
            ...
            #vjust = 5,
            inherit.aes = FALSE) +
  labs(title = "Catch Per Unit Effort L.lentjan in MAR Kapuntori",
       x = "",
       y = "CPUE (kg/fisher/trip)") +
  theme.default.1 +
  theme(
...
cpue.Kapuntori.indo <- ourfish.l_lentjan.cpue.1.plot + theme(
  plot.background = element_rect(fill = "transparent", colour = NA), # Makes the entire plot background transparent
  panel.background = element_rect(fill = "transparent", colour = NA) # Makes the panel background transparent
)
ggsave("cpue l_lentjan Kapuntori.svg", plot = cpue.Kapuntori.indo, bg = "transparent", width = 13.33, height = 7.3)
