...
#Katamba - Scylla serrata
ourfish.siotapina.s_serrata <- ourfish.siotapina.clean[species%in% "Scylla serrata",
                                                         .(count = sum(count)),
                                                         by = .(year_month, round_length)] [order(year_month)]
ourfish.siotapina.s_serrata$year_month <- as.Date(ourfish.siotapina.s_serrata$year_month)
ourfish.siotapina.s_serrata.wide <- ourfish.siotapina.s_serrata %>%
  pivot_wider(names_from = year_month,
              values_from = count,
              values_fill = 0)

ourfish.siotapina.s_serrata.sorted <- ourfish.siotapina.s_serrata.wide %>% arrange(round_length)

#===============================================================================
#Data arrangement: agar bisa dibaca sama R
dates <- colnames(ourfish.siotapina.s_serrata.sorted)[-1]
...
length.series.data <- list(dates = dates,
                           midLengths = ourfish.siotapina.s_serrata.sorted$round_length,
                           catch = as.matrix(ourfish.siotapina.s_serrata.sorted[,-1]))
...
max.length = as.numeric(max(ourfish.siotapina.s_serrata.sorted$round_length))
...
K.fishlife = 0.31 #Input K value from Fish Life script or other literatures/sources
...
length.series.data$M = 0.42

#preparing raw data for length series
ourfish.siotapina.s_serrata.long <- ourfish.siotapina.s_serrata.sorted %>%
  pivot_longer(
    cols = starts_with("20"),
    names_to = "dates",
    values_to = "value")

ourfish.siotapina.s_serrata.long.2 <- ourfish.siotapina.s_serrata.long %>%
  uncount(weights = value)

#Raw Data
length.raw <- tibble(Length = ourfish.siotapina.s_serrata.long.2$round_length)
...
ggsave("s_serrata size.svg", plot = size.structure.plot, bg = "transparent", width = 13.33, height = 7.3)

#total landings & CPUE L-lentjan================================================
ourfish.s_serrata.landings.1 <- ourfish.siotapina.clean[species %in% "Scylla serrata",
                                            .(sum_landings = sum(weight_kg)),
                                            by = .(date, species)][order(date)]

ourfish.s_serrata.landings.1.glm <- ourfish.s_serrata.landings.1 %>%
  nest(-'species') %>%
  mutate(model = map(data, ~ glm(sum_landings ~ date, data = .))) %>%
  mutate(tidied = map(model, tidy)) %>%
  unnest(tidied)

ourfish.s_serrata.landings.1.glm2 <- ourfish.s_serrata.landings.1.glm %>%
  dplyr::filter(term == "date") %>%
  mutate(p_value_adjusted = p.adjust(p.value)) %>%
  arrange(p_value_adjusted) %>%
  mutate(stat_test = ifelse(p_value_adjusted < 0.05, "significant", "not significant")) %>% #Pvalue <0.05 indicate ada perbedaan antara data dalam kelompok (signifikan)
...
setDT(ourfish.s_serrata.landings.1.glm2)

# Label Landings trends
ourfish.s_serrata.landings.1.glm3 <- ourfish.s_serrata.landings.1.glm2[,
                                                           trend := case_when(estimate >= 0 & stat_test == "significant" ~ "Increased", #estimate is to measure the alternative hypothesis, >=0 means there is positive linear relationship between predictor (date) and response (CPUE or weight)
                                                                              stat_test == "not significant" ~ "Stable",
                                                                              TRUE ~ "Decreased")]

trend.label <- ourfish.s_serrata.landings.1.glm3[, `:=`(term = NULL, estimate = NULL, std.error = NULL, statistic = NULL, p.value = NULL, p_value_adjusted = NULL, stat_test = NULL)]

summary(ourfish.s_serrata.landings.1.glm3)

ourfish.s_serrata.landings.1.plot <- ggplot(ourfish.s_serrata.landings.1, aes(x = date, y = sum_landings)) +
  ...
  geom_text(data = trend.label,
            ...
            #vjust = 5,
            inherit.aes = FALSE) +
  labs(title = "Total Landings S.serrata in MAR siotapina",
       x = "",
       y = "Total landings(kg)") +
  theme.default.1 +
  theme(
...
landings.siotapina.indo <- ourfish.s_serrata.landings.1.plot + theme(
  plot.background = element_rect(fill = "transparent", colour = NA), # Makes the entire plot background transparent
  panel.background = element_rect(fill = "transparent", colour = NA) # Makes the panel background transparent
)
ggsave("landings s_serrata siotapina.svg", plot = landings.siotapina.indo, bg = "transparent", width = 13.33, height = 7.3)


#### CPUE Trends in Southeast Sulawesi ####
# Subset weight data by date, species for total landings
ourfish.s_serrata.cpue.1 <- ourfish.siotapina.clean[species %in% "Scylla serrata",
                                                    .(sum_landings = sum(weight_kg)),
                                                    by = .(date, species, fisher_id)][order(date)]

ourfish.s_serrata.cpue.1.glm <- ourfish.s_serrata.cpue.1 %>%
  nest(-species) %>%
  mutate(model = map(data, ~ glm(sum_landings ~ date, data = .))) %>%
  mutate(tidied = map(model, tidy)) %>%
  unnest(tidied)

ourfish.s_serrata.cpue.1.glm2 <- ourfish.s_serrata.cpue.1.glm %>%
  dplyr::filter(term == "date") %>%
  mutate(p_value_adjusted = p.adjust(p.value)) %>%
  arrange(p_value_adjusted) %>%
  mutate(stat_test = ifelse(p_value_adjusted < 0.05, "significant", "not significant")) %>% #Pvalue <0.05 indicate ada perbedaan antara data dalam kelompok (signifikan)
...
setDT(ourfish.s_serrata.cpue.1.glm2)

# Label Landings trends
ourfish.s_serrata.cpue.1.glm3 <- ourfish.s_serrata.cpue.1.glm2[,
                                                   trend := case_when(estimate >= 0 & stat_test == "significant" ~ "Increased", #estimate is to measure the alternative hypothesis, >=0 means there is positive linear relationship between predictor (date) and response (CPUE or weight)
                                                                      stat_test == "not significant" ~ "Stable",
                                                                      TRUE ~ "Decreased")]

trend.label <- ourfish.s_serrata.cpue.1.glm3[, `:=`(term = NULL, estimate = NULL, std.error = NULL, statistic = NULL, p.value = NULL, p_value_adjusted = NULL, stat_test = NULL)]

summary(ourfish.s_serrata.cpue.1.glm3)

ourfish.s_serrata.cpue.1.plot <- ggplot(ourfish.s_serrata.cpue.1, aes(x = date, y = sum_landings)) +
  geom_smooth(method = "loess", span = 0.5, color = "#e78828", fill = "#e78828") +
  scale_y_continuous(expand = c(0,0), limits = c(0,NA), oob = squish) +
  scale_x_date(labels = date_format("%b %Y"),
               date_breaks = "6 month") +
  geom_text(data = trend.label,
            ...
            #vjust = 5,
            inherit.aes = FALSE) +
  labs(title = "Catch Per Unit Effort S.serrata in MAR siotapina",
       x = "",
       y = "CPUE (kg/fisher/trip)") +
  theme.default.1 +
  theme(
...
cpue.siotapina.indo <- ourfish.s_serrata.cpue.1.plot + theme(
  plot.background = element_rect(fill = "transparent", colour = NA), # Makes the entire plot background transparent
  panel.background = element_rect(fill = "transparent", colour = NA) # Makes the panel background transparent
)
ggsave("cpue s_serrata siotapina.svg", plot = cpue.siotapina.indo, bg = "transparent", width = 13.33, height = 7.3)
