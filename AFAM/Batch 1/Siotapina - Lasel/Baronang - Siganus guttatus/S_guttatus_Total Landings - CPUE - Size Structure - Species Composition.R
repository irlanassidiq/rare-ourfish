...
setwd("C:/Users/IrlanAssidiq/OneDrive - Rare/ourfish/AFAM/Siotapina - Lasel")
getwd()
...
#===============================================================================
# Total Landings
ourfish.siotapina<- ourfish.raw[#month >= 2 & month <= 7 &
  ma_name %in% "Siotapina - Lasel"]

ourfish.ses.landings.siotapina <- ourfish.siotapina[ma_name %in% "Siotapina - Lasel",
                                                  .(Berat = sum(weight_kg)),
                                                  by = .(date, buying_unit, label)][order(date)]

setnames(ourfish.siotapina, "ma_name", "location")
ourfish.ses.landings.1 <- ourfish.siotapina[location %in% "Siotapina - Lasel",
...
ourfish.ses.landings.1.plot <- ggplot(ourfish.ses.landings.1, aes(x = date, y = sum_landings)) +
  ...
  geom_text(data = trend.label,
            ...
            #vjust = 5,
            inherit.aes = FALSE) +
  labs(title = "Total Landings in MAR siotapina",
       x = "",
       y = "Total landings(kg)") +
  theme.default.1 +
  theme(
...
landings.siotapina <- ourfish.ses.landings.1.plot + theme(
  plot.background = element_rect(fill = "transparent", colour = NA), # Makes the entire plot background transparent
  panel.background = element_rect(fill = "transparent", colour = NA) # Makes the panel background transparent
)
ggsave("landings siotapina.svg", plot = landings.siotapina, bg = "transparent", width = 13.33, height = 7.3)


#### CPUE Trends in Southeast Sulawesi ####
# Subset weight data by date, location for total landings
ourfish.ses.cpue.1 <- ourfish.siotapina[location %in% "Siotapina - Lasel",
                                       .(sum_landings = sum(weight_kg)),
...
ourfish.ses.cpue.1.plot <- ggplot(ourfish.ses.cpue.1, aes(x = date, y = sum_landings)) +
  ...
  geom_text(data = trend.label,
            aes(x = as.Date("2022-06-01", format("%Y-%m-%d")),
                ...
                label = trend),
            hjust = 0.1,
            color = "#B50A37",  #Orange #e78828; Hijau #00833e; Tosca #06a7b4; Merah #B50A37
            size = 6,
            #vjust = 5,
            inherit.aes = FALSE) +
  labs(title = "Catch Per Unit Effort in MAR siotapina",
       x = "Month",
...
cpue.siotapina <- ourfish.ses.cpue.1.plot + theme(
  plot.background = element_rect(fill = "transparent", colour = NA), # Makes the entire plot background transparent
  panel.background = element_rect(fill = "transparent", colour = NA) # Makes the panel background transparent
)
ggsave("cpue siotapina.svg", plot = cpue.siotapina, bg = "transparent", width = 13.33, height = 7.3)

# Detecting outliers in weight data
library(EnvStats)
quantile(ourfish.siotapina$weight_kg, 0.01)
quantile(ourfish.siotapina$weight_kg, 0.99)

# Number of k value for Rosner Test
num.k.upper.weight <- ourfish.siotapina[weight_kg >= quantile(weight_kg, 0.99), .N]
num.k.lower.weight <- ourfish.siotapina[weight_kg <= quantile(weight_kg, 0.01), .N]
k.value.rosnertest.weight <- num.k.upper.weight + num.k.lower.weight

# Statistical test for outliers using Rosner Test
rosner.test.landings.data <- rosnerTest(ourfish.siotapina$weight_kg, k = k.value.rosnertest.weight)
...
ourfish.siotapina.outlier <- ourfish.siotapina[landings.data.outliers$Obs.Num, ]
...
ourfish.siotapina.clean <- ourfish.siotapina[-landings.data.outliers$Obs.Num, ]

## Box Plot for annual comparison

# Make data for two years period
ourfish.siotapina.clean.boxplot.1 <- ourfish.siotapina.clean[,
                                                     year_period := as.factor("2 Years Period")]

# Make data for Year 1
ourfish.siotapina.clean.boxplot.2 <- ourfish.siotapina.clean[date >= start.date & date <= "2022-10-01"][,
                                                                                                year_period := as.factor("Year 1")]

# Make data for Year 2
ourfish.siotapina.clean.boxplot.3 <- ourfish.siotapina.clean[date >= "2022-08-01" & date <= end.date][,
                                                                                              year_period := as.factor("Year 2")]

# Combine data of two years period with year 1 & 2 data
ourfish.siotapina.clean.boxplot.4 <- rbind(ourfish.siotapina.clean.boxplot.1, ourfish.siotapina.clean.boxplot.2, ourfish.siotapina.clean.boxplot.3)

# Annual CPUE & Catch Value using box plot
ourfish.siotapina.clean.boxplot.5 <- ourfish.siotapina.clean.boxplot.4[,
...
ourfish.siotapina.clean.boxplot.5[,
...
t.test(data = ourfish.siotapina.clean.boxplot.5[!year_period == "2 Years Period"],
...
boxplot.annual.cpue <- 
  ggplot(data = ourfish.siotapina.clean.boxplot.5,
...
ourfish.siotapina.clean.boxplot.6 <- ourfish.siotapina.clean.boxplot.5[,
                                                               .(n_trip = .N,
...
### Average length
ourfish.siotapina.clean$ind_weight_g <- (ourfish.siotapina.clean$weight_kg *1000)/ourfish.siotapina.clean$count
ourfish.siotapina.clean$length <- (ourfish.siotapina.clean$ind_weight_g/ourfish.siotapina.clean$a)^(1/ourfish.siotapina.clean$b)

### average length by family
ourfish.siotapina.prov.ma$ind_weight_g <- (ourfish.siotapina.prov.ma$weight_kg *1000)/ourfish.siotapina.prov.ma$count
ourfish.siotapina.prov.ma$length <- (ourfish.siotapina.prov.ma$ind_weight_g/ourfish.siotapina.prov.ma$a)^(1/ourfish.siotapina.prov.ma$b)


# Subset weight data by date, location, fisher_id, family
ourfish.siotapina.length.fam.1 <- ourfish.siotapina.prov.ma[,
                                                    .(mean_length_fam = mean(length)),
                                                    by = .(date, location, family)][order(date)]

ourfish.siotapina.length.sp.1 <- ourfish.siotapina.prov.ma[,
                                                   .(mean_length_sp = mean(length)),
                                                   by = .(date, location,family, species)][order(date)]

ourfish.siotapina.length.sp.1 <- filter_if(ourfish.siotapina.length.sp.1, is.numeric, all_vars((.) != 0))
table.sp.length <- table(c(ourfish.siotapina.length.sp.1$species,
                           ourfish.siotapina.length.sp.1$mean_length_sp))

# Rename level1_name and ma_name into the same column name to be combined
ourfish.siotapina.combine.1 <- ourfish.siotapina.clean[ , ]
ourfish.siotapina.combine.2 <- ourfish.siotapina.clean[ , ]

# Combine data from whole siotapina with data by MA+R
ourfish.siotapina.prov.ma <- rbind(ourfish.siotapina.combine.1, ourfish.siotapina.combine.2)
...
table.fam <- table(ourfish.siotapina.prov.ma$family)

# Change data into wide format
ourfish.siotapina.length.fam.1.wide <- dcast(ourfish.siotapina.length.fam.1,
                                         formula = date + location ~ family,
                                         value.var = "mean_length_fam")

ourfish.siotapina.length.sp.1.wide <- dcast(ourfish.siotapina.length.sp.1,
                                        formula = date + location + family ~ species,
                                        value.var = "mean_length_sp")

# Replace NA value with 0
ourfish.siotapina.length.fam.1.wide[is.na(ourfish.siotapina.length.fam.1.wide)] <- 0

ourfish.siotapina.length.sp.1.wide[is.na(ourfish.siotapina.length.sp.1.wide)] <- 0

# Change data back into long format
ourfish.siotapina.length.fam.1.long <- melt(ourfish.siotapina.length.fam.1.wide,
                                        measure.vars = c(3:20), # Please check the number of column first
                                        variable.name = "family",
                                        value.name = "mean_length_fam")
ourfish.siotapina.length.fam.1.long.clean <- filter_if(ourfish.siotapina.length.fam.1.long, is.numeric, all_vars((.) !=0))
ourfish.siotapina.length.fam.1.long.clean <- filter_if(ourfish.siotapina.length.fam.1.long.clean, is.numeric, all_vars((.) !=Inf))
ourfish.siotapina.length.sp.1.long <- melt(ourfish.siotapina.length.sp.1.wide,
                                       measure.vars = c(4:45), # Please check the number of column first
                                       variable.name = "species",
                                       value.name = "mean_length_sp")

ourfish.siotapina.length.sp.1.long.clean <- filter_if(ourfish.siotapina.length.sp.1.long, is.numeric, all_vars((.) != 0))
ourfish.siotapina.length.sp.1.long.clean <- filter_if(ourfish.siotapina.length.sp.1.long.clean, is.numeric, all_vars((.) !=Inf)) 

table.sp.clean <- table(ourfish.siotapina.length.sp.1.long.clean$species)

#Size Structure
#creating new column for Year_month
library(zoo)
ourfish.siotapina.clean$year_month <- ourfish.siotapina.clean$date
ourfish.siotapina.clean$year_month <- as.yearmon(ourfish.siotapina.clean$year_month, format = "%Y_%m")

#creating midlength, using round up function
ourfish.siotapina.clean$round_length <- round(ourfish.siotapina.clean$length)

#Katamba - Siganus guttatus
ourfish.siotapina.s_guttatus <- ourfish.siotapina.clean[species%in% "Siganus guttatus",
                                                         .(count = sum(count)),
                                                         by = .(year_month, round_length)] [order(year_month)]
ourfish.siotapina.s_guttatus$year_month <- as.Date(ourfish.siotapina.s_guttatus$year_month)
ourfish.siotapina.s_guttatus.wide <- ourfish.siotapina.s_guttatus %>%
  pivot_wider(names_from = year_month,
              values_from = count,
              values_fill = 0)

ourfish.siotapina.s_guttatus.sorted <- ourfish.siotapina.s_guttatus.wide %>% arrange(round_length)

#===============================================================================
#Data arrangement: agar bisa dibaca sama R
dates <- colnames(ourfish.siotapina.s_guttatus.sorted)[-1]
...
length.series.data <- list(dates = dates,
                           midLengths = ourfish.siotapina.s_guttatus.sorted$round_length,
                           catch = as.matrix(ourfish.siotapina.s_guttatus.sorted[,-1]))
...
max.length = as.numeric(max(ourfish.siotapina.s_guttatus.sorted$round_length))
...
K.fishlife = 1.4 #Input K value from Fish Life script or other literatures/sources
...
length.series.data$M = 1.4

#preparing raw data for length series
ourfish.siotapina.s_guttatus.long <- ourfish.siotapina.s_guttatus.sorted %>%
  pivot_longer(
    cols = starts_with("20"),
    names_to = "dates",
    values_to = "value")

ourfish.siotapina.s_guttatus.long.2 <- ourfish.siotapina.s_guttatus.long %>%
  uncount(weights = value)

#Raw Data
length.raw <- tibble(Length = ourfish.siotapina.s_guttatus.long.2$round_length)
...
ggsave("s_guttatus size.svg", plot = size.structure.plot, bg = "transparent", width = 13.33, height = 7.3)

#total landings & CPUE L-lentjan================================================
ourfish.s_guttatus.landings.1 <- ourfish.siotapina.clean[species %in% "Siganus guttatus",
                                            .(sum_landings = sum(weight_kg)),
                                            by = .(date, species)][order(date)]

ourfish.s_guttatus.landings.1.glm <- ourfish.s_guttatus.landings.1 %>%
  nest(-'species') %>%
  mutate(model = map(data, ~ glm(sum_landings ~ date, data = .))) %>%
  mutate(tidied = map(model, tidy)) %>%
  unnest(tidied)

ourfish.s_guttatus.landings.1.glm2 <- ourfish.s_guttatus.landings.1.glm %>%
  dplyr::filter(term == "date") %>%
  mutate(p_value_adjusted = p.adjust(p.value)) %>%
  arrange(p_value_adjusted) %>%
  mutate(stat_test = ifelse(p_value_adjusted < 0.05, "significant", "not significant")) %>% #Pvalue <0.05 indicate ada perbedaan antara data dalam kelompok (signifikan)
...
setDT(ourfish.s_guttatus.landings.1.glm2)

# Label Landings trends
ourfish.s_guttatus.landings.1.glm3 <- ourfish.s_guttatus.landings.1.glm2[,
                                                           trend := case_when(estimate >= 0 & stat_test == "significant" ~ "Increased", #estimate is to measure the alternative hypothesis, >=0 means there is positive linear relationship between predictor (date) and response (CPUE or weight)
                                                                              stat_test == "not significant" ~ "Stable",
                                                                              TRUE ~ "Decreased")]

trend.label <- ourfish.s_guttatus.landings.1.glm3[, `:=`(term = NULL, estimate = NULL, std.error = NULL, statistic = NULL, p.value = NULL, p_value_adjusted = NULL, stat_test = NULL)]

summary(ourfish.s_guttatus.landings.1.glm3)

ourfish.s_guttatus.landings.1.plot <- ggplot(ourfish.s_guttatus.landings.1, aes(x = date, y = sum_landings)) +
  ...
  geom_text(data = trend.label,
            ...
            #vjust = 5,
            inherit.aes = FALSE) +
  labs(title = "Total Landings s.guttatus in MAR siotapina",
       x = "",
       y = "Total landings(kg)") +
  theme.default.1 +
  theme(
...
landings.siotapina.indo <- ourfish.s_guttatus.landings.1.plot + theme(
  plot.background = element_rect(fill = "transparent", colour = NA), # Makes the entire plot background transparent
  panel.background = element_rect(fill = "transparent", colour = NA) # Makes the panel background transparent
)
ggsave("landings s_guttatus siotapina.svg", plot = landings.siotapina.indo, bg = "transparent", width = 13.33, height = 7.3)


#### CPUE Trends in Southeast Sulawesi ####
# Subset weight data by date, species for total landings
ourfish.s_guttatus.cpue.1 <- ourfish.siotapina.clean[species %in% "Siganus guttatus",
                                                    .(sum_landings = sum(weight_kg)),
                                                    by = .(date, species, fisher_id)][order(date)]

ourfish.s_guttatus.cpue.1.glm <- ourfish.s_guttatus.cpue.1 %>%
  nest(-species) %>%
  mutate(model = map(data, ~ glm(sum_landings ~ date, data = .))) %>%
  mutate(tidied = map(model, tidy)) %>%
  unnest(tidied)

ourfish.s_guttatus.cpue.1.glm2 <- ourfish.s_guttatus.cpue.1.glm %>%
  dplyr::filter(term == "date") %>%
  mutate(p_value_adjusted = p.adjust(p.value)) %>%
  arrange(p_value_adjusted) %>%
  mutate(stat_test = ifelse(p_value_adjusted < 0.05, "significant", "not significant")) %>% #Pvalue <0.05 indicate ada perbedaan antara data dalam kelompok (signifikan)
...
setDT(ourfish.s_guttatus.cpue.1.glm2)

# Label Landings trends
ourfish.s_guttatus.cpue.1.glm3 <- ourfish.s_guttatus.cpue.1.glm2[,
                                                   trend := case_when(estimate >= 0 & stat_test == "significant" ~ "Increased", #estimate is to measure the alternative hypothesis, >=0 means there is positive linear relationship between predictor (date) and response (CPUE or weight)
                                                                      stat_test == "not significant" ~ "Stable",
                                                                      TRUE ~ "Decreased")]

trend.label <- ourfish.s_guttatus.cpue.1.glm3[, `:=`(term = NULL, estimate = NULL, std.error = NULL, statistic = NULL, p.value = NULL, p_value_adjusted = NULL, stat_test = NULL)]

summary(ourfish.s_guttatus.cpue.1.glm3)

ourfish.s_guttatus.cpue.1.plot <- ggplot(ourfish.s_guttatus.cpue.1, aes(x = date, y = sum_landings)) +
  geom_smooth(method = "loess", span = 0.5, color = "#e78828", fill = "#e78828") +
  scale_y_continuous(expand = c(0,0), limits = c(0,NA), oob = squish) +
  scale_x_date(labels = date_format("%b %Y"),
               date_breaks = "6 month") +
  geom_text(data = trend.label,
            ...
            #vjust = 5,
            inherit.aes = FALSE) +
  labs(title = "Catch Per Unit Effort s.guttatus in MAR siotapina",
       x = "",
       y = "CPUE (kg/fisher/trip)") +
  theme.default.1 +
  theme(
...
cpue.siotapina.indo <- ourfish.s_guttatus.cpue.1.plot + theme(
  plot.background = element_rect(fill = "transparent", colour = NA), # Makes the entire plot background transparent
  panel.background = element_rect(fill = "transparent", colour = NA) # Makes the panel background transparent
)
ggsave("cpue s_guttatus siotapina.svg", plot = cpue.siotapina.indo, bg = "transparent", width = 13.33, height = 7.3)
