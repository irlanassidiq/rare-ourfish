...
### Average length
ourfish.Bolaang_Mongondow.clean$ind_weight_g <- (ourfish.Bolaang_Mongondow.clean$weight_kg *1000)/ourfish.Bolaang_Mongondow.clean$count
ourfish.Bolaang_Mongondow.clean$length <- (ourfish.Bolaang_Mongondow.clean$ind_weight_g/ourfish.Bolaang_Mongondow.clean$a)^(1/ourfish.Bolaang_Mongondow.clean$b)

### average length by family
ourfish.Bolaang_Mongondow.prov.ma$ind_weight_g <- (ourfish.Bolaang_Mongondow.prov.ma$weight_kg *1000)/ourfish.Bolaang_Mongondow.prov.ma$count
ourfish.Bolaang_Mongondow.prov.ma$length <- (ourfish.Bolaang_Mongondow.prov.ma$ind_weight_g/ourfish.Bolaang_Mongondow.prov.ma$a)^(1/ourfish.Bolaang_Mongondow.prov.ma$b)
...
ourfish.Bolaang_Mongondow.l_malabaricus <- ourfish.Bolaang_Mongondow.clean[species%in% "Lutjanus malabaricus",
                                                         .(count = sum(count)),
                                                         by = .(year_month, round_length)] [order(year_month)]
ourfish.Bolaang_Mongondow.l_malabaricus$year_month <- as.Date(ourfish.Bolaang_Mongondow.l_malabaricus$year_month)
ourfish.Bolaang_Mongondow.l_malabaricus.wide <- ourfish.Bolaang_Mongondow.l_malabaricus %>%
  pivot_wider(names_from = year_month,
              values_from = count,
              values_fill = 0)

ourfish.Bolaang_Mongondow.l_malabaricus.sorted <- ourfish.Bolaang_Mongondow.l_malabaricus.wide %>% arrange(round_length)

#===============================================================================
#Data arrangement: agar bisa dibaca sama R
dates <- colnames(ourfish.Bolaang_Mongondow.l_malabaricus.sorted)[-1]
...
length.series.data <- list(dates = dates,
                           midLengths = ourfish.Bolaang_Mongondow.l_malabaricus.sorted$round_length,
                           catch = as.matrix(ourfish.Bolaang_Mongondow.l_malabaricus.sorted[,-1]))
...
max.length = as.numeric(max(ourfish.Bolaang_Mongondow.l_malabaricus.sorted$round_length))
...
K.fishlife = 0.170 #Input K value from Fish Life script or other literatures/sources
...
length.series.data$M = 0.25

#preparing raw data for length series
ourfish.Bolaang_Mongondow.l_malabaricus.long <- ourfish.Bolaang_Mongondow.l_malabaricus.sorted %>%
  pivot_longer(
    cols = starts_with("20"),
    names_to = "dates",
    values_to = "value")

ourfish.Bolaang_Mongondow.l_malabaricus.long.2 <- ourfish.Bolaang_Mongondow.l_malabaricus.long %>%
  uncount(weights = value)

#Raw Data
length.raw <- tibble(Length = ourfish.Bolaang_Mongondow.l_malabaricus.long.2$round_length)
...
#total landings & CPUE L-malabaricus================================================
ourfish.l_malabaricus.landings.1 <- ourfish.Bolaang_Mongondow.clean[species %in% "Lutjanus malabaricus",
                                            .(sum_landings = sum(weight_kg)),
...
ourfish.l_malabaricus.landings.1.plot <- ggplot(ourfish.l_malabaricus.landings.1, aes(x = date, y = sum_landings)) +
  ...
  geom_text(data = trend.label,
            ...
            #vjust = 5,
            inherit.aes = FALSE) +
  labs(title = "Total Landings L.malabaricus in MAR Bolaang_Mongondow",
       x = "",
       y = "Total landings(kg)") +
  theme.default.1 +
  theme(
...
landings.Bolaang_Mongondow.indo <- ourfish.l_malabaricus.landings.1.plot + theme(
  plot.background = element_rect(fill = "transparent", colour = NA), # Makes the entire plot background transparent
  panel.background = element_rect(fill = "transparent", colour = NA) # Makes the panel background transparent
)
ggsave("landings l_malabaricus Bolaang_Mongondow.svg", plot = landings.Bolaang_Mongondow.indo, bg = "transparent", width = 13.33, height = 7.3)


#### CPUE Trends in Southeast Sulawesi ####
# Subset weight data by date, species for total landings
ourfish.l_malabaricus.cpue.1 <- ourfish.Bolaang_Mongondow.clean[species %in% "Lutjanus malabaricus",
                                                    .(sum_landings = sum(weight_kg)),
...
ourfish.l_malabaricus.cpue.1.plot <- ggplot(ourfish.l_malabaricus.cpue.1, aes(x = date, y = sum_landings)) +
  ...
  geom_text(data = trend.label,
            ...
            #vjust = 5,
            inherit.aes = FALSE) +
  labs(title = "Catch Per Unit Effort L.malabaricus in MAR Bolaang_Mongondow",
       x = "",
       y = "CPUE (kg/fisher/trip)") +
  theme.default.1 +
  theme(
...
cpue.Bolaang_Mongondow.indo <- ourfish.l_malabaricus.cpue.1.plot + theme(
  plot.background = element_rect(fill = "transparent", colour = NA), # Makes the entire plot background transparent
  panel.background = element_rect(fill = "transparent", colour = NA) # Makes the panel background transparent
)
ggsave("cpue l_malabaricus Bolaang_Mongondow.svg", plot = cpue.Bolaang_Mongondow.indo, bg = "transparent", width = 13.33, height = 7.3)
